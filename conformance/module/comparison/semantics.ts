...
export function unevaluatedRule(obligation: ProofObligation, module: ExpectedModule): VerificationRule {
  return {
    id: obligation.id,
    status: 'error',
    diagnostics: [
...
export function declarationPrefix(declaration: ExpectedDeclaration): string {
...
export function matchesExternalType(
  expected: Extract<ExpectedTypeExpression, { kind: 'external' }>,
  ...
  return (
    ...
    coordinates.some(
      (coordinate) =>
        coordinate === expected.target || coordinate.startsWith(`${expected.target}/`),
    )
  )
}

export function matchesExternalIdentity(
  expected: Extract<ExpectedTypeExpression, { kind: 'external' }>,
  observed: Extract<ObservedType, { kind: 'reference' }>,
): boolean {
  return (
    expected.target.startsWith('platform:') &&
    observed.name === expected.name &&
    observed.identity === `${expected.target}#${expected.name}`
  )
}

export function externalAliasMatches(expected: ExpectedType, observed: ObservedDeclaration): boolean {
  return (
    expected.expression.kind === 'external' && matchesExternalType(expected.expression, observed)
  )
}

export function expectedDeclarationAliasesObserved(
...
export function heritageProof(expected: ExpectedType): string {
...
export function expectedTypeName(expected: ExpectedType): string {
  const expression = expected.expression
  return expression.kind === 'declaration'
...
export function withoutUndefined(type: ObservedType): ObservedType {
...
export function memberType(member: ObservedMember | undefined): ObservedType | undefined {
...
export function expectedReadonlyShape(
  type: Extract<ExpectedTypeExpression, { readonly kind: 'external' }>,
): boolean {
  return (
    type.target === 'platform:typescript' && type.name === 'Readonly' && type.arguments.length === 1
  )
}

export function structuralExpectedMembers(
  type: ExpectedTypeExpression | undefined,
): readonly ExpectedMember[] | undefined {
  let current = type
...
export function expectedBooleanUnion(type: ExpectedTypeExpression): boolean {
  if (type.kind !== 'union' || type.types.length !== 2) return false
  const values = type.types.flatMap((item) =>
    item.kind === 'literal' && typeof item.value === 'boolean' ? [item.value] : [],
  )
  return values.length === 2 && values.includes(false) && values.includes(true)
}

export function flattenExpectedTypeSet(
  kind: 'union' | 'intersection',
  values: readonly ExpectedTypeExpression[],
): readonly ExpectedTypeExpression[] {
  const flattened = values.flatMap((value): readonly ExpectedTypeExpression[] =>
    value.kind === kind ? flattenExpectedTypeSet(kind, value.types) : [value],
  )
  return flattened.filter((value) => !expectedLiteralIsSubsumed(value, flattened))
}

export function expectedLiteralIsSubsumed(
  value: ExpectedTypeExpression,
  values: readonly ExpectedTypeExpression[],
): boolean {
...
export function expectedContainsBooleanLiterals(values: readonly ExpectedTypeExpression[]): boolean {
  if (values.some((value) => value.kind === 'data' && value.data === 'boolean')) return false
  let hasFalse = false
  let hasTrue = false
  for (const value of values) {
    if (value.kind !== 'literal' || typeof value.value !== 'boolean') continue
    if (value.value) hasTrue = true
    else hasFalse = true
  }
  return hasFalse && hasTrue
}

export function removeSubsumedObservedLiterals(values: readonly ObservedType[]): readonly ObservedType[] {
    ...
    const primitive =
      typeof value.value === 'string'
        ...
        : typeof value.value === 'number'
          ...
          : 'boolean'
    return !values.some(
      (candidate) => candidate.kind === 'primitive' && candidate.name === primitive,
    )
  })
}

export function collapseObservedUnion(
  expected: ExpectedTypeExpression,
  values: readonly ObservedType[],
...
export function expectedTypeParameterKey(scope: string, index: number): string {
  return `${scope}:${index}`
}

export function substituteExpectedTypeParameters(
  type: ExpectedTypeExpression,
  bindings: ReadonlyMap<string, ExpectedTypeExpression>,
): ExpectedTypeExpression {
  switch (type.kind) {
    case 'parameter':
    ...
    case 'external':
      return {
        ...
        arguments: type.arguments.map((argument) =>
          substituteExpectedTypeParameters(argument, bindings),
        ),
      }
    case 'template':
      return {
    ...
    case 'constructor':
      return {
        ...
        ...(type.overloads
          ? {
              overloads: type.overloads.map((callable) =>
                substituteExpectedCallableTypeParameters(callable, bindings),
              ),
            }
          : {}),
      }
...
export function substituteExpectedCallableTypeParameters(
  callable: ExpectedCallableType,
  bindings: ReadonlyMap<string, ExpectedTypeExpression>,
): ExpectedCallableType {
  return {
    ...callable,
          ...
          typeParameters: callable.typeParameters.map((parameter) => ({
            ...
            ...(parameter.constraint
              ? {
                  constraint: {
                    ...parameter.constraint,
                    expression: substituteExpectedTypeParameters(
...
export function substituteTypeParameters(
  type: ObservedType,
  scope: string,
  arguments_: readonly ObservedType[],
): ObservedType {
  switch (type.kind) {
    case 'parameter':
    ...
    case 'reference':
      return {
        ...type,
        arguments: type.arguments.map((argument) =>
          substituteTypeParameters(argument, scope, arguments_),
        ),
      }
    case 'template':
      return {
    ...
    case 'constructor':
      return {
        ...
        ...(type.overloads
          ? {
              overloads: type.overloads.map((callable) =>
                substituteCallableTypeParameters(callable, scope, arguments_),
              ),
            }
          : {}),
      }
...
export function reduceTransparentObservedType(type: ObservedType): ObservedType {
  let current = type
  const active = new Set<ObservedType>()
  while (current.kind === 'conditional' && !active.has(current)) {
    active.add(current)
    if (current.check.kind === 'never') return { kind: 'never' }
...
export function substituteCallableTypeParameters(
  callable: ObservedCallable,
  scope: string,
  arguments_: readonly ObservedType[],
): ObservedCallable {
  return {
...
export function sameObservedType(
  left: ObservedType,
  right: ObservedType,
...
export function sameObservedTypes(
  left: readonly ObservedType[],
  right: readonly ObservedType[],
...
export function sameObservedTypeSet(
  left: readonly ObservedType[],
  right: readonly ObservedType[],
  scopes: ReadonlyMap<string, string>,
): boolean {
  ...
  return left.every((item) => {
    const index = right.findIndex(
      (candidate, candidateIndex) =>
        !used.has(candidateIndex) && sameObservedType(item, candidate, scopes),
    )
...
export function sameObservedMembers(
  left: readonly ObservedMember[],
  right: readonly ObservedMember[],
  scopes: ReadonlyMap<string, string>,
): boolean {
  if (left.length !== right.length) return false
  const used = new Set<number>()
...
export function sameObservedCallable(
  left: ObservedCallable,
  right: ObservedCallable,
  parentScopes: ReadonlyMap<string, string>,
): boolean {
  ...
  for (const [index, parameter] of leftParameters.entries()) {
    ...
    if (previous !== undefined && previous !== target.scope) return false
...
export function sameObservedCallables(
  left: readonly ObservedCallable[] | undefined,
  right: readonly ObservedCallable[] | undefined,
  scopes: ReadonlyMap<string, string>,
): boolean {
...
export function intrinsicTypeCompatible(
  expected: 'null' | 'undefined' | 'void' | 'never',
  observed: ObservedType['kind'],
): boolean {
  return expected === observed || (expected === 'void' && observed === 'undefined')
}

export function dataTypeCompatible(expected: DataType, observed: ObservedType): boolean {
  const expectedName = expected === 'decimal' ? 'number' : expected
  if (observed.kind === 'primitive' && observed.name === expectedName) return true
  return Boolean(
    expected === 'bytes' &&
...
export function packagePatternMatches(pattern: string, packageName: string): boolean {
...
export function typeSetAssignments(
  candidates: readonly (readonly number[])[],
  limit: number,
): readonly (readonly number[])[] {
  const output: number[][] = []
  const assignment = Array<number>(candidates.length).fill(-1)
  const used = new Set<number>()
  const visit = (): void => {
...
export function witnessSkeletonCompatible(
  expected: ExpectedTypeExpression,
  observed: ObservedType,
): boolean {
  if (expected.kind !== observed.kind) return false
  if (expected.kind !== 'function' || observed.kind !== 'function') return true
  if (expected.callable.mode !== observed.callable.mode) return false
  if (expected.callable.parameters.length !== observed.callable.parameters.length) return false
...
export function mergeRule(
  ...
  if (priority(status) < priority(rule.status)) rule.status = status
  if (diagnostic && !rule.diagnostics.some((item) => sameDiagnostic(item, diagnostic))) {
    rule.diagnostics.push(diagnostic)
  }
}

export function sameDiagnostic(left: VerificationDiagnostic, right: VerificationDiagnostic): boolean {
  return (
...
export function priority(status: VerificationStatus): number {
...
export function expectedDeclarationReferences(
  declaration: ExpectedDeclaration,
): readonly DeclarationIdentity[] {
  const references: DeclarationIdentity[] = []
  const visitType = (type: ExpectedType | undefined): void => {
    if (type) visitExpression(type.expression)
  }
  const visitTypeParameters = (parameters: ExpectedDeclaration['typeParameters']): void => {
  ...
  const visitCallable = (
    callable: Extract<ExpectedTypeExpression, { kind: 'function' | 'constructor' }>['callable'],
  ): void => {
    visitTypeParameters(callable.typeParameters)
    for (const parameter of callable.parameters) visitType(parameter)
    visitType(callable.returns)
  }
  const visitExpression = (expression: ExpectedTypeExpression): void => {
    switch (expression.kind) {
      ...
      case 'external':
        ...
        return
      case 'template':
        for (const type of expression.types) visitExpression(type)
        return
      case 'array':
      ...
      case 'indexed-access':
        ...
        return
      case 'object':
        for (const member of expression.members) visitType(member)
        return
      case 'function':
      case 'constructor':
        visitCallable(expression.callable)
        for (const overload of expression.overloads ?? []) visitCallable(overload)
        return
      default:
        return
    }
  ...
  if (declaration.alias) references.push(declaration.alias)
  visitTypeParameters(declaration.typeParameters)
  visitType(declaration.valueType)
  for (const member of declaration.fields ?? []) visitType(member)
  for (const member of declaration.properties ?? []) visitType(member)
  for (const member of declaration.callables ?? []) references.push(member.callable)
  for (const member of declaration.statics ?? []) references.push(member.callable)
  for (const type of declaration.extends ?? []) visitType(type)
  for (const type of declaration.implements ?? []) visitType(type)
  for (const parameter of declaration.parameters ?? []) visitType(parameter)
  visitType(declaration.returns ?? undefined)
  for (const overload of declaration.overloads ?? []) visitCallable(overload)
  if (declaration.facets?.type.valueType) visitType(declaration.facets.type.valueType)
