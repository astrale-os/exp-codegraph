...
import {
  ...
  observedDeclarationLookupKey,
} from '../context.ts'

import {
  declarationPrefix,
  matchesExternalType,
  externalAliasMatches,
  expectedDeclarationAliasesObserved,
  heritageProof,
  expectedTypeName,
  withoutUndefined,
  memberType,
  structuralExpectedMembers,
  expectedTypeParameterKey,
  substituteExpectedTypeParameters,
  substituteTypeParameters,
  reduceTransparentObservedType,
  witnessSkeletonCompatible,
  safeId,
  expectedDeclarationReferences,
} from '../semantics.ts'

import { TypeEvaluator } from '../type/evaluator.ts'

export abstract class DeclarationEvaluator extends TypeEvaluator {
  protected bindDeclaration(
    expectedIdentity: DeclarationIdentity,
    observed: ObservedDeclaration,
    ruleId: string,
    projection: 'full' | 'type' | 'identity' = 'full',
  ): void {
    ...
    if (!expectedDeclaration) {
      ...
      this.error(
        ruleId,
        'MODULE_EXPECTED_DECLARATION_UNRESOLVED',
        `Compiled declaration identity is unavailable: ${expectedIdentity.key}`,
    ...
    if (
      expectedDeclaration.conformance === 'identity' ||
    ...
    if (
      ...
      this.error(
        ruleId,
        'MODULE_IMPORTED_DECLARATION_UNBOUND',
        `The owning module has not established a code identity for ${expectedIdentity.name}.`,
        expectedIdentity.pointer,
        observed.location,
        canonical,
        observed.identity,
    ...
    if (
      ...
      this.fail(
        ...
        `Canonical declaration identity differs for ${expectedIdentity.name}.`,
        expectedIdentity.pointer,
        observed.location,
        previous,
        observed.identity,
    ...
    this.pairs.add(pair)
    if (projection === 'type') this.compareProjectedTypeDeclaration(expectedDeclaration, observed)
    else if (projection === 'identity') {
      this.pass(declarationPrefix(expectedDeclaration))
      this.coverIdentityClosure(observed.identity)
    } else this.compareDeclaration(expectedDeclaration, observed)
  }

  protected blockOnDependency(
    ruleId: string,
    expectedIdentity: DeclarationIdentity,
    unavailable: {
      readonly id: string
    ...
    if (this.blockedDependencies.has(unavailable.id)) return
  ...
  protected compareProjectedTypeDeclaration(
    ...
    if (expected.identity.kind !== 'value') {
      this.fail(
        prefix,
        'MODULE_DECLARATION_KIND_MISMATCH',
        `Type-only export does not expose a type-alias facet for ${expected.identity.name}.`,
        expected.pointer,
        observed.location,
      )
    ...
    if (expected.valueType) {
      ...
      if (externalAliasMatches(expected.valueType, observed)) {
        ...
        this.fail(
          id,
          'MODULE_TYPE_FACET_MISSING',
          `Type-only export does not expose materializable type evidence for ${expected.identity.name}.`,
  ...
  protected compareDeclaration(expected: ExpectedDeclaration, observed: ObservedDeclaration): void {
    const prefix = declarationPrefix(expected)
    ...
    if (!declarationKindCompatible(expected.identity.kind, observed, expected)) {
      this.fail(
        prefix,
        'MODULE_DECLARATION_KIND_MISMATCH',
        `Declaration kind differs for ${expected.identity.name}.`,
        expected.pointer,
        observed.location,
    ...
    if (expected.identity.kind === 'value') {
      if (expected.valueType) {
        ...
        if (externalAliasMatches(expected.valueType, observed)) {
          ...
          this.fail(
            id,
            'MODULE_VALUE_TYPE_MISSING',
            `The public value type is unavailable: ${expected.identity.name}`,
            expected.valueType.pointer,
            observed.location,
          )
        } else if (
          this.compareType(expected.valueType, observed.valueType, id, observed.location)
        ) {
          this.pass(id)
        }
      }
      const observedFields =
        observed.kind === 'interface' ? observed.properties : observed.fields
      this.compareMembers(
        expected,
        observed,
        'field',
      ...
      this.compareHeritage(
        expected,
        observed,
        'extends',
        expected.extends,
        observed.kind === 'interface' ? observed.extends : [],
      )
      return
    }
    if (expected.identity.kind === 'callable') {
      if (!observed.callable) {
        this.failDeclarationChildren(
          expected,
          observed.location,
  ...
  protected compareCallSignature(expected: ExpectedDeclaration, observed: ObservedDeclaration): void {
    const prefix = declarationPrefix(expected)
    const expectedCallable = expected.returns !== undefined || Boolean(expected.overloads?.length)
    if (!expectedCallable) {
      if (observed.callable) {
        this.addInverse(
          `observed.call-signature.${safeId(observed.identity)}`,
          'fail',
          `Public call signature is not declared: ${observed.name}().`,
          observed.callable.location,
          'MODULE_CALL_SIGNATURE_UNDECLARED',
        )
      }
      return
    }
    ...
    if (!observed.callable) {
      this.fail(
        id,
        'MODULE_CALL_SIGNATURE_MISSING',
        `Specified call signature is absent: ${expected.identity.name}().`,
        expected.pointer,
        observed.location,
      )
      for (const obligation of this.expected.obligations) {
        if (
          obligation.id.startsWith(`${prefix}.parameter.`) ||
  ...
  protected compareTypeParameters(
    expected: ExpectedDeclaration,
    observed: ObservedDeclaration,
    options?: {
      readonly prefix?: string
      readonly expectedParameters?: NonNullable<ExpectedDeclaration['typeParameters']>
      readonly observedParameters?: NonNullable<ObservedDeclaration['typeParameters']>
    },
  ): void {
    ...
    for (let index = 0; index < maximum; index++) {
      ...
      if (!parameter) {
        this.addInverse(
          `observed.type-parameter.${safeId(inverseIdentity)}.${index}`,
          'fail',
          `Public generic declaration has an undeclared type parameter at position ${index}.`,
          actual?.location,
          'MODULE_TYPE_PARAMETER_UNDECLARED',
        )
        continue
      }
      ...
      if (!actual) {
        this.fail(
          id,
          'MODULE_TYPE_PARAMETER_MISSING',
          `Generic type parameter is absent at position ${index}.`,
          parameter.pointer,
          observed.location,
        )
        continue
      ...
      if (parameter.variance !== actual.variance) {
        this.fail(
          id,
          'MODULE_TYPE_PARAMETER_VARIANCE_MISMATCH',
          `Generic type parameter variance differs at position ${index}.`,
          parameter.pointer,
          actual.location,
          parameter.variance,
          actual.variance,
      ...
      if (Boolean(parameter.const) !== Boolean(actual.const)) {
        this.fail(
          id,
          'MODULE_TYPE_PARAMETER_CONST_MISMATCH',
          `Generic type parameter const inference differs at position ${index}.`,
          parameter.pointer,
          actual.location,
          Boolean(parameter.const),
          Boolean(actual.const),
        )
        continue
      }
      if (Boolean(parameter.constraint) !== Boolean(actual.constraint)) {
        this.fail(
          id,
          'MODULE_TYPE_PARAMETER_CONSTRAINT_MISMATCH',
          `Generic type parameter constraint differs at position ${index}.`,
          parameter.pointer,
          actual.location,
          parameter.constraint?.expression,
          actual.constraint,
        )
        continue
      }
      ...
      if (Boolean(parameter.default) !== Boolean(actual.default)) {
        this.fail(
          id,
          'MODULE_TYPE_PARAMETER_DEFAULT_MISMATCH',
          `Generic type parameter default differs at position ${index}.`,
          parameter.pointer,
          actual.location,
      ...
      this.addInverse(
        `observed.type-parameter.${safeId(inverseIdentity)}.${index}`,
        'pass',
        `Observed generic type parameter is declared at position ${index}.`,
        actual.location,
      )
    }
  }

  protected compareDeclarationFacets(
    expected: ExpectedDeclaration,
    observed: ObservedDeclaration,
  ): void {
    const expectedFacets = expected.facets
    if (!expectedFacets) return
    const prefix = declarationPrefix(expected)
    ...
    if (!observedType) {
      this.fail(
        typeId,
        'MODULE_TYPE_FACET_MISSING',
        `Type-alias facet is absent: ${expected.identity.name}`,
        expectedFacets.type.pointer,
        observed.location,
      )
      if (expectedFacets.type.valueType) {
        this.fail(
          typeValueId,
          'MODULE_TYPE_FACET_MISSING',
          `Type-alias facet is unavailable for comparison: ${expected.identity.name}`,
          expectedFacets.type.valueType.pointer,
          observed.location,
        )
      }
    } else {
      ...
      if (expectedFacets.type.valueType) {
        if (expectedFacets.type.conformance === 'identity') {
          ...
          this.compareType(
            expectedFacets.type.valueType,
            observedType.valueType,
            typeValueId,
            observedType.location,
          )
        ) {
          this.pass(typeValueId)
        }
    ...
    if (!observedValue) {
      this.fail(
        valueId,
        'MODULE_VALUE_FACET_MISSING',
        `Runtime value facet is absent: ${expected.identity.name}`,
        expectedFacets.value.pointer,
        observed.location,
      )
      return
    }
    ...
    if (expectedFacets.value.kind === 'value') {
      ...
      if (observedValue.kind !== 'value') {
        this.fail(
          valueTypeId,
          'MODULE_VALUE_FACET_KIND_MISMATCH',
          `Runtime value facet is callable but an object-valued constructor namespace is required: ${expected.identity.name}`,
          expectedFacets.value.valueType.pointer,
        ...
        this.compareType(
          expectedFacets.value.valueType,
    ...
    if (observedValue.kind !== 'callable') {
      this.fail(
        valueId,
        'MODULE_VALUE_FACET_KIND_MISMATCH',
        `Runtime value facet is not callable: ${expected.identity.name}`,
        expectedFacets.value.pointer,
        observedValue.location,
      )
      return
    ...
    const selfFactorySignature =
      signature?.identity.key === expected.identity.key && signature.facets !== undefined
    if (!signature || (signature.identity.kind !== 'callable' && !selfFactorySignature)) {
      this.error(
        valueId,
        'MODULE_FACTORY_REFERENCE_INVALID',
        `Factory signature declaration is unavailable: ${expectedFacets.value.callable.key}`,
        expectedFacets.value.pointer,
        observedValue.location,
      )
      return
    ...
    if (!selfFactorySignature) this.pass(declarationPrefix(signature))
  ...
  protected compareMembers(
    ...
    observedDeclaration: ObservedDeclaration,
    category: 'field' | 'property',
    expectedMembers: readonly ExpectedMember[] | undefined,
    observedMembers: readonly ObservedMember[] | undefined,
  ): void {
    const observed = observedMembers ?? []
    const matchedObserved = new Set<ObservedMember>()
    for (const member of expectedMembers ?? []) {
      ...
      if (!actual?.type) {
        this.fail(
          ...
          `Specified ${category} is absent: ${declaration.identity.name}.${member.name}`,
          member.pointer,
          actual?.location,
        )
        continue
      ...
      if (member.optional !== actual.optional) {
        this.fail(
          id,
          'MODULE_OPTIONALITY_MISMATCH',
          `Optionality differs for ${declaration.identity.name}.${member.name}.`,
          member.pointer,
          actual.location,
          member.optional,
          actual.optional,
      ...
      if (member.readonly !== undefined && member.readonly !== actual.readonly) {
        this.fail(
          id,
          'MODULE_READONLY_MISMATCH',
          `Readonly semantics differ for ${declaration.identity.name}.${member.name}.`,
          member.pointer,
          actual.location,
          member.readonly,
          actual.readonly,
      ...
      if (this.compareType(member, actualType, id, actual.location)) this.pass(id)
    }
    for (const actual of observed) {
      ...
      if (matchedObserved.has(actual)) {
        this.addInverse(
          inverseId,
          'pass',
          `Observed ${category} is declared: ${actual.name}`,
          actual.location,
        )
      } else {
        this.addInverse(
          inverseId,
          'fail',
          `Public ${category} is not declared: ${observedDeclaration.name}.${actual.name}`,
          actual.location,
          'MODULE_MEMBER_UNDECLARED',
        )
      }
    }
  }
  ...
  protected matchMember(
    ...
    used: ReadonlySet<ObservedMember>,
  ): ObservedMember | undefined {
    if (expected.key === 'named') {
      return observed.find(
        (member) => !used.has(member) && member.key === 'named' && member.name === expected.name,
      )
    }
    ...
    const candidates = observed.filter((member) => {
      if (used.has(member) || member.key !== 'unique-symbol' || !member.type) return false
      if (expected.optional !== member.optional) return false
      if (expected.readonly !== undefined && expected.readonly !== member.readonly) return false
      return true
    })
    ...
    return (
      candidates.find((member) =>
        witnessSkeletonCompatible(
          expected.expression,
          member.optional ? withoutUndefined(member.type!) : member.type!,
        ),
      ) ?? candidates[0]
    )
  }

  protected compareCallableMembers(
    declaration: ExpectedDeclaration,
    observedDeclaration: ObservedDeclaration,
    category: 'callable' | 'static',
    expectedMembers: readonly ExpectedCallableMember[] | undefined,
    observedMembers: readonly ObservedMember[] | undefined,
  ): void {
    ...
    for (const member of expectedMembers ?? []) {
      ...
      if (!actual?.callable) {
        this.fail(
          id,
          'MODULE_CALLABLE_MEMBER_MISSING',
          `Specified ${category} is absent: ${declaration.identity.name}.${member.name}`,
          member.pointer,
          actual?.location,
        )
      ...
      if (Boolean(member.optional) !== actual.optional) {
        this.fail(
          id,
          'MODULE_CALLABLE_OPTIONALITY_MISMATCH',
          `Callable member optionality differs: ${declaration.identity.name}.${member.name}`,
          member.pointer,
          actual.location,
          Boolean(member.optional),
          actual.optional,
        )
      }
      this.pass(id)
      ...
      if (!signature || signature.identity.kind !== 'callable') {
        this.error(
          id,
          'MODULE_CALLABLE_REFERENCE_INVALID',
          `Callable signature declaration is unavailable: ${member.callable.key}`,
          member.pointer,
          actual.location,
        )
        continue
  ...
  protected compareCallableSignatures(
    expected: ExpectedDeclaration,
    observed: ObservedCallable,
    observedOverloads?: readonly ObservedCallable[],
    options?: {
      readonly expectedTypeParameters?: readonly ExpectedTypeParameter[]
      readonly typeParameterPrefix?: string
    },
  ): void {
    ...
    if (!expectedOverloads?.length) {
      if (actual.length > 1) {
        this.addInverse(
          `observed.overload.${safeId(declarationPrefix(expected))}`,
          'fail',
          `Public callable has ${actual.length} overloads but the contract declares one signature.`,
          actual[1]?.location ?? observed.location,
          'MODULE_OVERLOAD_UNDECLARED',
        )
      }
    ...
    for (let index = 0; index < maximum; index++) {
      ...
      if (!specified) {
        this.addInverse(
          `observed.overload.${safeId(prefix)}.${index}`,
          'fail',
          `Public callable has an undeclared overload at position ${index}.`,
      ...
      if (!implemented) {
        this.fail(
          id,
          'MODULE_OVERLOAD_MISSING',
          `Specified overload is absent at position ${index}.`,
          pointer,
          observed.location,
        )
        continue
      }
      if (this.compareCallableType(specified, implemented, id, pointer)) this.pass(id)
    }
  }

  protected compareCallableType(
    ...
    if (
      ...
      this.fail(
        ruleId,
        'MODULE_OVERLOAD_SIGNATURE_MISMATCH',
        'Callable overload mode, arity, or generic arity differs.',
        pointer,
        observed.location,
        expected,
        observed,
    ...
    for (const [index, parameter] of expectedTypeParameters.entries()) {
      ...
      if (!sameSkeleton) {
        this.fail(
          ruleId,
          'MODULE_OVERLOAD_TYPE_PARAMETER_MISMATCH',
          `Callable overload type parameter differs at position ${index}.`,
          parameter.pointer,
          actual.location,
        )
        matches = false
    ...
    for (const [index, parameter] of expected.parameters.entries()) {
      ...
      if (parameter.optional !== actual.optional || Boolean(parameter.rest) !== actual.rest) {
        this.fail(
          ruleId,
          'MODULE_OVERLOAD_PARAMETER_MISMATCH',
          `Callable overload parameter differs at position ${index}.`,
          parameter.pointer,
          actual.location,
        )
        matches = false
    ...
    if (
      !this.compareTypeExpression(
        expected.returns.expression,
        observed.returns,
        ruleId,
        observed.location,
        expected.returns.pointer,
      )
    ) {
      matches = false
    }
  ...
  protected compareCallable(
    ...
    const prefix = declarationPrefix(expected)
    this.compareCallableTypeParameters(
      expected,
      observed,
      options?.typeParameterPrefix ?? prefix,
      options?.expectedTypeParameters,
    )
    ...
    for (let index = 0; index < maximum; index++) {
      ...
      if (!parameter) {
        this.addInverse(
          `observed.parameter.${safeId(prefix)}.${index}.${safeId(actual!.name)}`,
          'fail',
          `Public callable has an undeclared parameter at position ${index}: ${actual!.name}`,
          actual!.location,
          'MODULE_PARAMETER_UNDECLARED',
        )
        continue
      }
      ...
      if (!actual) {
        ...
        this.fail(
          id,
          'MODULE_PARAMETER_MISSING',
          `Callable parameter is absent at position ${index}: ${parameter.name}`,
          parameter.pointer,
          observed.location,
        )
        continue
      ...
      if (parameter.optional !== actual.optional) {
        this.fail(
          id,
          'MODULE_PARAMETER_OPTIONALITY_MISMATCH',
          `Callable parameter optionality differs: ${parameter.name}`,
          parameter.pointer,
          actual.location,
          parameter.optional,
          actual.optional,
      ...
      if (parameter.rest !== undefined && parameter.rest !== actual.rest) {
        this.fail(
          id,
          'MODULE_PARAMETER_REST_MISMATCH',
          `Callable parameter rest semantics differ: ${parameter.name}`,
          parameter.pointer,
          actual.location,
          parameter.rest,
          actual.rest,
      ...
      const actualType = actual.optional ? withoutUndefined(actual.type) : actual.type
      if (this.compareType(parameter, actualType, id, actual.location)) this.pass(id)
      this.addInverse(
        `observed.parameter.${safeId(prefix)}.${index}.${safeId(actual.name)}`,
        'pass',
        `Observed parameter is declared: ${actual.name}`,
        actual.location,
      )
    }
    if (expected.returns !== undefined) {
      ...
      if (expected.returns === null) {
        if (observed.returns.kind === 'void') this.pass(id)
        else
          this.fail(
            id,
            'MODULE_RETURN_TYPE_MISMATCH',
            'Callable return type differs.',
            `${expected.pointer}/returns`,
            observed.location,
            'void',
            observed.returns,
          )
    ...
    if (expected.mode) {
      ...
      if (expected.mode === observed.mode) this.pass(id)
      else
        this.fail(
          id,
          'MODULE_CALLABLE_MODE_MISMATCH',
          'Callable sync/async mode differs.',
          `${expected.pointer}/mode`,
          observed.location,
          expected.mode,
          observed.mode,
        )
  ...
  protected compareCallableTypeParameters(
    expected: ExpectedDeclaration,
    observed: ObservedCallable,
    prefix: string,
    expectedParameters?: readonly ExpectedTypeParameter[],
  ): void {
    ...
    for (let index = 0; index < maximum; index++) {
      ...
      if (!parameter || !observedParameter) {
        ...
        this.fail(
          id,
          !parameter ? 'MODULE_TYPE_PARAMETER_UNDECLARED' : 'MODULE_TYPE_PARAMETER_MISSING',
          `Callable type parameter differs at position ${index}.`,
          parameter?.pointer ?? expected.pointer,
          observedParameter?.location ?? observed.location,
        )
        continue
      }
      const id = `${prefix}.type-parameter.${index}`
      const constraintMatches =
        Boolean(parameter.constraint) === Boolean(observedParameter.constraint) &&
        (!parameter.constraint ||
          !observedParameter.constraint ||
          this.typeShapeCompatible(parameter.constraint.expression, observedParameter.constraint))
      const defaultMatches =
        Boolean(parameter.default) === Boolean(observedParameter.default) &&
        (!parameter.default ||
          !observedParameter.default ||
          this.typeShapeCompatible(parameter.default.expression, observedParameter.default))
      if (
        parameter.variance === observedParameter.variance &&
        ...
        this.fail(
          id,
          'MODULE_TYPE_PARAMETER_MISMATCH',
          `Callable type parameter differs at position ${index}.`,
          parameter.pointer,
          observedParameter.location,
        )
      }
    }
  }

  protected compareHeritage(
    declaration: ExpectedDeclaration,
    observedDeclaration: ObservedDeclaration,
    category: 'extends' | 'implements',
    expectedValues: readonly ExpectedType[] | undefined,
    observedValues: readonly string[] | undefined,
  ): void {
    ...
    for (const expected of expectedValues ?? []) {
      ...
      const candidates = [...remaining].filter((identity) => {
        ...
        if (!target) {
          return (
            expression.kind === 'external' && identity === `${expression.target}#${expression.name}`
          )
        }
        if (expression.kind === 'external') return matchesExternalType(expression, target)
        if (expression.kind !== 'declaration') return false
      ...
      if (!actual) {
        this.fail(
          id,
          'MODULE_HERITAGE_MISSING',
          `Heritage declaration is absent or ambiguous: ${expectedTypeName(expected)}`,
          expected.pointer,
          observedDeclaration.location,
        )
        continue
      }
      if (
      ...
      if (!observedTarget) {
        ...
        this.error(
          id,
          'MODULE_HERITAGE_UNRESOLVED',
          `Heritage target could not be observed: ${actual}`,
          expected.pointer,
          observedDeclaration.location,
        )
        continue
      ...
      if (!this.evaluated.has(id)) this.pass(id)
    }
    for (const actual of remaining) {
      const target = this.context.observedDeclarations.get(actual)
      this.addInverse(
        `observed.heritage.${safeId(observedDeclaration.identity)}.${category}.${safeId(actual)}`,
        'fail',
        `Public ${category} target is not declared: ${target?.name ?? actual}`,
        target?.location ?? observedDeclaration.location,
        'MODULE_HERITAGE_UNDECLARED',
      )
    }
  }

  protected coverIdentityClosure(identity: string): void {
    const pending = [identity]
    while (pending.length) {
      const current = pending.pop()!
      if (this.identityCovered.has(current)) continue
      ...
      if (declaration) pending.push(...declaration.referencedDeclarations)
    }
  }
   ...
   */
  protected coverExpectedReferenceClosure(root: ExpectedDeclaration): void {
    if (root.identity.source !== this.expected.id) return
    const pending = [root]
    const visited = new Set<string>()
    while (pending.length) {
      const declaration = pending.pop()!
      if (visited.has(declaration.identity.key)) continue
      ...
      for (const reference of expectedDeclarationReferences(declaration)) {
        ...
        if (target?.identity.source === this.expected.id) pending.push(target)
      }
    }
  }

  protected markAliasDeclarations(identity: string): void {
   ...
   */
  protected observedIdentitiesEquivalent(left: string, right: string): boolean {
    if (left === right) return true
    const leftClosure = this.observedAliasClosure(left)
    const rightClosure = this.observedAliasClosure(right)
    for (const identity of leftClosure) if (rightClosure.has(identity)) return true
    return false
  }

  /** Compare polymorphic-this through the declaration binding already proven for its owner. */
  protected thisOwnerCompatible(expectedOwner: string, observedOwner: string): boolean {
  ...
  protected observedAliasesExpectedDeclaration(
    identity: string,
    expected: ExpectedDeclaration,
  ): boolean {
    for (const current of this.observedAliasClosure(identity)) {
      const declaration = this.context.observedDeclarations.get(current)
      if (
        declaration?.name === expected.identity.name &&
  ...
  protected expectedDeclarationShape(
    ...
    if (!declaration || reference.arguments.length > parameters.length) return
    const bindings = new Map<string, ExpectedTypeExpression>()
    ...
    const declaredShape =
      declaration?.facets?.type.valueType?.expression ?? declaration?.valueType?.expression
    const shape = declaredShape
      ? substituteExpectedTypeParameters(declaredShape, bindings)
      : undefined
    if (
      shape?.kind === 'declaration' &&
  ...
  protected bindExpandedDeclarationReference(
    reference: Extract<ExpectedTypeExpression, { readonly kind: 'declaration' }>,
    ...
    if (!shape || !this.typeShapeCompatible(shape, observed)) return false
    const canonical = canonicalExpected(reference.declaration.key, this.context.expected)
    const expected = this.context.expected.declarations.get(canonical)
    if (!expected) return false
    const candidates = (
      this.context.observedDeclarationsByOwnerAndName.get(
        observedDeclarationLookupKey(expected.identity.source, expected.identity.name),
      ) ?? []
    ).filter((candidate) => declarationKindCompatible(expected.identity.kind, candidate, expected))
    ...
    if (previous && !this.observedIdentitiesEquivalent(previous, candidate.identity)) {
      this.fail(
        ruleId,
        'MODULE_DECLARATION_IDENTITY_MISMATCH',
        `Canonical declaration identity differs for ${reference.declaration.name}.`,
        pointer,
        location,
        previous,
        candidate.identity,
  ...
  protected observedAliasClosure(identity: string): ReadonlySet<string> {
  ...
  protected coverObservedAliasClosure(identity: string): void {
    for (const current of this.observedAliasClosure(identity)) this.boundObserved.add(current)
  }

  /** Expand a named type only while comparing it to an explicitly structural contract. */
  protected transparentObservedType(observed: ObservedType): ObservedType | undefined {
    if (observed.kind !== 'reference') return undefined
    if (
      observed.name === 'Readonly' &&
    ...
    let expanded: ObservedType | undefined = authored
    ...
    if (!expanded) expanded = declaration.facets?.type.valueType ?? declaration.valueType
    if (!expanded && declaration.kind === 'interface' && !declaration.extends?.length) {
      const members = [...(declaration.properties ?? []), ...(declaration.callables ?? [])].map(
    ...
    if (!scope || parameters.some((parameter) => parameter.scope !== scope)) return undefined
  ...
  protected resolveObservedIndexedAccess(
    indexed: Extract<ObservedType, { kind: 'indexed-access' }>,
  ): ObservedType | undefined {
    if (indexed.index.kind !== 'literal' || typeof indexed.index.value !== 'string') return undefined
    ...
    while (object.kind === 'reference' && !active.has(object.identity)) {
      ...
      const member = [...(declaration.properties ?? []), ...(declaration.fields ?? [])].find(
        (candidate) => candidate.key === 'named' && candidate.name === key,
      )
  ...
  protected isLocalAliasReference(observed: ObservedType): boolean {
  ...
  protected failDeclarationChildren(
    declaration: ExpectedDeclaration,
    actual: ImplementationLocation | undefined,
    message: string,
  ): void {
    for (const obligation of this.expected.obligations) {
      if (obligation.id === declarationPrefix(declaration)) continue
      if (!obligation.id.startsWith(`${declarationPrefix(declaration)}.`)) continue
      this.set(obligation.id, 'fail', {
