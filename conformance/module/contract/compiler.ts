...
import { declarationKey, MODULE_CONTRACT_ID } from './model.ts'
import { compileObligations } from './obligations.ts'
...
interface ContractModule {
  ...
  readonly packages: SpecificationSnapshot['module']['packageAuthority']['packages']
  readonly packagePatterns: SpecificationSnapshot['module']['packageAuthority']['packagePatterns']
  readonly api: NonNullable<SpecificationSnapshot['module']['api']> & {
    readonly model: NonNullable<NonNullable<SpecificationSnapshot['module']['api']>['model']>
  }
}

interface ContractDeclarationOwner {
  readonly module: ContractModule
  readonly declaration: ObservedDeclaration
}

interface ContractIndex {
  readonly owner: ReadonlyMap<string, ContractDeclarationOwner>
...
export function compileModuleContract(snapshot: SpecificationSnapshot): ModuleCompilation {
  ...
  const identities = new Map<string, DeclarationIdentity>()
  for (const declaration of module.api.model.surface.declarations) {
    const owner = index.owner.get(declaration.identity)
    if (owner) identities.set(declaration.identity, identityOf(owner.module, declaration))
  }
  const local = module.api.model.surface.declarations.filter(
    (declaration) => index.owner.get(declaration.identity)?.module.id === module.id,
  )
  const compiler = new ApiModuleCompiler(module, index, identities, diagnostics)
  const compilation = compiler.compile(local)
  if (!compilation.module || diagnostics.length) return compilation
...
class ApiModuleCompiler {
  ...
  readonly identities: ReadonlyMap<string, DeclarationIdentity>
  readonly diagnostics: Diagnostic[]
  readonly imports = new Map<string, DeclarationIdentity>()
  readonly externalExports = new Map<string, DeclarationIdentity>()
  readonly pending: ExpectedDeclaration[] = []
  readonly locations: Record<string, { file: string; line: number; column: number }> = {}

  constructor(
    module: ContractModule,
    index: ContractIndex,
    identities: ReadonlyMap<string, DeclarationIdentity>,
  ...
  compile(local: readonly ObservedDeclaration[]): ModuleCompilation {
    ...
    if (this.diagnostics.length) return { diagnostics: this.diagnostics }
    const declarationMap = new Map(
      declarations.map((declaration) => [declaration.identity.key, declaration]),
    )
    ...
    const expected: ExpectedModule = {
      ...
      packages,
      packagePatterns,
      obligations: compileObligations(
        name,
        `${this.module.declarationPointer}/api`,
        declarationMap,
        exports,
        [...this.imports.values()],
        packages,
  ...
  declaration(observed: ObservedDeclaration): ExpectedDeclaration {
    ...
    const factoryConformance =
      typeFacetMetadata?.conformance === 'identity' &&
    ...
    const common = {
      ...
      pointer,
      conformance:
        observed.kind === 'factory'
          ? factoryConformance
          : (metadata?.conformance ?? ('exact' as const)),
    }
    if (common.conformance === 'identity' && observed.kind !== 'factory') return common
    const typeParameters =
      observed.typeParameters?.length && typeFacetMetadata?.conformance !== 'identity'
      ...
      return {
        ...
        facets: {
          type: {
            ...
            ...typeParameters,
            valueType: this.expectedType(
              observed.facets.type.authoredValueType ?? observed.facets.type.valueType,
              `${pointer}/facet/type/value`,
            ),
          },
          value:
    ...
    if (observed.kind === 'interface' || observed.kind === 'class') {
      return {
        ...
        ...(observed.callable
          ...
          : {}),
        properties: (observed.properties ?? []).flatMap((member) =>
          member.type ? [this.member(member, pointer)] : [],
        ),
        callables: (observed.callables ?? []).flatMap((member) =>
          member.callable ? [this.callableMember(member, pointer, observed.identity)] : [],
        ),
        ...(observed.kind === 'class'
          ? {
              statics: (observed.statics ?? []).flatMap((member) =>
                member.callable
          ...
          : {}),
        extends: (observed.extends ?? []).map((identity, index) =>
          this.referenceType(identity, `${pointer}/extends/${index}`),
        ),
        implements: (observed.implements ?? []).map((identity, index) =>
          this.referenceType(identity, `${pointer}/implements/${index}`),
        ),
      }
    }
    const alias =
      metadata?.form === 'type-alias' &&
    ...
    return {
      ...
      ...(observed.valueType && !alias
        ...
        : {}),
      fields: (observed.fields ?? []).flatMap((member) =>
        member.type ? [this.member(member, pointer)] : [],
      ),
      callables: (observed.callables ?? []).flatMap((member) =>
        member.callable ? [this.callableMember(member, pointer, observed.identity)] : [],
  ...
  externalExport(
    item: ObservedExport,
  ): { readonly identity: DeclarationIdentity; readonly sourceModule: string } | undefined {
    if (!item.sourceModule?.startsWith('package:')) return
    ...
    if (!identity) {
      const pointer = `${this.module.declarationPointer}/api/external/${escapePointer(
        `${item.sourceModule}#${declaration.name}`,
      )}`
      identity = {
      ...
      this.pending.push({
        ...
        ...(declaration.kind === 'factory' ? { factory: true as const } : {}),
      })
  ...
  member(member: ObservedMember, ownerPointer: string): ExpectedMember {
  ...
  typeParameters(
    parameters: NonNullable<ObservedDeclaration['typeParameters']>,
    ownerPointer: string,
  ) {
  ...
  parameters(callable: ObservedCallable, ownerPointer: string): ExpectedParameter[] {
  ...
  callableFields(
    ...
    overloads: readonly ObservedCallable[] | undefined,
    ownerPointer: string,
    separateTypeParameters = false,
  ) {
    if (overloads && overloads.length > 1) {
      return {
        overloads: overloads.map((signature, index) =>
          this.callableType(signature, `${ownerPointer}/overload/${index}`),
        ),
      }
    }
  ...
  callableType(callable: ObservedCallable, pointer: string): ExpectedCallableType {
  ...
  expectedType(type: ObservedType, pointer: string): ExpectedType {
    return { expression: this.type(type), optional: false, pointer }
  }

  type(type: ObservedType): ExpectedTypeExpression {
    switch (type.kind) {
      case 'primitive':
      ...
      case 'function':
        return {
          ...
          ...(type.overloads && type.overloads.length > 1
            ? {
                overloads: type.overloads.map((callable, index) =>
                  this.callableType(callable, `/api/function/overload/${index}`),
                ),
              }
            : {}),
        }
      case 'constructor':
        return {
          ...
          ...(type.overloads && type.overloads.length > 1
            ? {
                overloads: type.overloads.map((callable, index) =>
                  this.callableType(callable, `/api/constructor/overload/${index}`),
                ),
              }
            : {}),
        }
  ...
  referenceType(identity: string, pointer: string): ExpectedType {
  ...
  referenceIdentity(identity: string): DeclarationIdentity | undefined {
    const local = this.identities.get(identity)
    if (local) {
      if (local.source !== this.module.id) this.imports.set(local.key, local)
      return local
    }
    const owner = this.index.owner.get(identity)
    if (!owner) return
    if (
      owner.module.id !== this.module.id &&
      !this.index.exportsByModule.get(owner.module.id)?.has(identity)
    ) {
      this.report(
        'API_IMPORTED_DECLARATION_PRIVATE',
        `Imported declaration is not explicitly exported by ${owner.module.name}: ${owner.declaration.name}`,
      )
      return
    }
  ...
  record(pointer: string, location: { file?: string; line: number; column: number }): void {
...
function externalTypeTarget(coordinate: string): string {
  ...
  if (!canonical.startsWith('package:')) return canonical
...
function externalReferenceTarget(
  declaration: ObservedDeclaration | undefined,
  identity: string,
): string {
  const coordinate = declaration?.packageCoordinate ?? declaration?.location.external
  if (coordinate) return externalTypeTarget(coordinate)
  if (identity.startsWith('platform:')) return identity.split('#')[0]!
  return 'unowned'
}

function identityOf(module: ContractModule, declaration: ObservedDeclaration): DeclarationIdentity {
...
function indexSnapshotApi(module: ContractModule): ContractIndex {
  ...
  const modules = new Map<string, ContractModule>([[module.id, module]])
  ...
  const owner = new Map<string, ContractDeclarationOwner>()
  ...
  for (const item of declarations) {
    ...
    if (values) values.add(item.identity)
    else exported.set(owned.id, new Set([item.identity]))
  }
  exported.set(
    module.id,
...
function declarationModuleId(declaration: ObservedDeclaration): string | undefined {
...
function canonicalTypeProviderCoordinate(coordinate: string): string {
...
function declarationKind(declaration: ObservedDeclaration): DeclarationKind {
  if (declaration.kind === 'callable') return 'callable'
  if (declaration.kind === 'interface') return 'interface'
  if (declaration.kind === 'class') return 'class'
  return 'value'
}
