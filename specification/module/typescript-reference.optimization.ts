...
const canonicalPaths = operationSnapshotNamespace<string>('module-typescript-canonical-paths')

/** Canonicalize a physical source once per coherent compiler operation. */
export function canonicalModuleTypeScriptPath(path: string): string {
...
export function deduplicateModuleSourceReferences(
  references: readonly ModuleSourceReference[],
): readonly ModuleSourceReference[] {
