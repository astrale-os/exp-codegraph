...
import { markdownInlineCodeSpans } from '../../markdown/render.ts'
...
export function markdownSemanticMentions(
  document: MarkdownDocument,
): readonly MarkdownSemanticMention[] {
  return markdownInlineCodeSpans(document).flatMap((span) => {
    if (span.linked) return []
...
function semanticMention(value: string): { label: string; call: boolean } | undefined {
  if (value.length === 0 || value.length > MAX_SEMANTIC_CODE_CHARACTERS) return
  const label = semanticPath.exec(value)?.[0]
  if (!label) return
  const suffix = value.slice(label.length)
  if (!suffix) return { label, call: false }
...
function balancedCall(value: string): boolean {
  ...
  for (let index = 0; index < value.length; index++) {
    ...
    if (closing[character]) brackets.push(closing[character])
    else if (character === ')' || character === ']' || character === '}') {
      if (brackets.pop() !== character) return false
      if (brackets.length === 0 && index !== value.length - 1) return false
    }
  }
