...
export function compileLayout(source: string, text: string): LayoutCompilation {
  ...
  for (const statement of file.statements) {
    if (ts.isImportDeclaration(statement)) {
      if (
        ...
        diagnostics.push(
          nodeDiagnostic(
            'LAYOUT_IMPORT_INVALID',
            `Layout files may import only defineLayout from ${AUTHORING_SPECIFIER}.`,
            source,
            file,
            statement,
          ),
      ...
      if (!bindings || !ts.isNamedImports(bindings)) {
        diagnostics.push(
          nodeDiagnostic(
            'LAYOUT_IMPORT_INVALID',
            'Layout files must use a named defineLayout import.',
            source,
            file,
            statement,
          ),
      ...
      for (const binding of bindings.elements) {
        if ((binding.propertyName?.text ?? binding.name.text) !== 'defineLayout') {
          diagnostics.push(
            nodeDiagnostic(
              'LAYOUT_IMPORT_INVALID',
              'Layout files may import only defineLayout.',
              source,
              file,
              binding,
            ),
    ...
    diagnostics.push(
      nodeDiagnostic(
        'LAYOUT_STATEMENT_INVALID',
        'Layout files may contain only an authoring import and one default export.',
        source,
        file,
        statement,
      ),
  ...
  if (!literal) {
    if (assignment) {
      diagnostics.push(
        nodeDiagnostic(
          'LAYOUT_DEFINITION_INVALID',
          'The default export must call defineLayout with one array or configuration literal.',
          source,
          file,
          assignment,
        ),
  ...
  if (!entriesArray) return { entries: [], exact, ignore, diagnostics }
  if (entriesArray.elements.length === 0) {
    diagnostics.push(
      nodeDiagnostic(
        'LAYOUT_EMPTY',
        'Remove layout.ts when no physical ownership boundary is specified.',
        source,
        file,
        entriesArray,
      ),
  ...
  const entries: CompiledLayoutEntry[] = []
  const paths = new Map<string, CompiledLayoutEntry>()
  for (const element of entriesArray.elements) {
    ...
    if (path === undefined) {
      diagnostics.push(
        nodeDiagnostic(
          ...
          'Every layout entry must be a string path literal.',
          source,
          file,
          element,
        ),
    ...
    if (path === './') {
      diagnostics.push(
        nodeDiagnostic(
          'LAYOUT_ROOT_IMPLICIT',
          'The module root is implicit; remove the ./ layout entry.',
          source,
          file,
          element,
        ),
    ...
    if (!kind) {
      diagnostics.push(
        nodeDiagnostic(
          'LAYOUT_PATH_INVALID',
          'Layout paths must be canonical, relative POSIX paths; directories end with one slash.',
          source,
          file,
          element,
        ),
    ...
    if (selfSpecifyingPath(path)) {
      diagnostics.push(
        nodeDiagnostic(
          'LAYOUT_PATH_RESERVED',
          'layout.ts cannot govern .spec/ or .history/.',
          source,
          file,
          element,
        ),
    ...
    const entry: CompiledLayoutEntry = {
      path,
      kind,
    ...
    if (duplicate) {
      diagnostics.push(
        nodeDiagnostic(
          'LAYOUT_PATH_DUPLICATE',
          `Layout path ${path} is already declared.`,
          source,
          file,
          element,
        ),
  ...
  const stems = new Map<string, CompiledLayoutEntry>()
  ...
  for (const entry of entries) {
    for (const parent of parentDirectories(entry.path)) {
      ...
      if (declaration?.kind === 'directory') continue
      diagnostics.push({
...
export async function observeLayout(
  catalogRoot: string,
  moduleRoot: string,
  source: string,
  entries: readonly CompiledLayoutEntry[],
  options: { readonly exact?: boolean; readonly ignore?: readonly string[] } = {},
): Promise<LayoutConformance> {
  const root = resolve(catalogRoot)
  const module = resolve(moduleRoot)
  const actual = new Map<string, LayoutObservedKind>()
  const observationDiagnostics: Diagnostic[] = []
  const ignore = effectiveIgnorePatterns(options.ignore ?? [])
  const scanPolicy: LayoutScanPolicy = {
    declared: new Set(entries.map((entry) => pathStem(entry.path))),
    requiredDirectories: new Set(
      entries.flatMap((entry) => parentDirectories(entry.path).map(pathStem)),
    ),
  ...
  const actualByStem = new Map(
    [...actual].map(([path, kind]) => [pathStem(path), { path, kind }] as const),
  )
  const declaredByStem = new Map(entries.map((entry) => [pathStem(entry.path), entry] as const))
  ...
  const observedByPath = new Map(observedEntries.map((entry) => [entry.path, entry] as const))
  const diagnostics: Diagnostic[] = [...observationDiagnostics]
  for (const entry of entries) {
    const observation = observedByPath.get(entry.path)!
    if (observation.status === 'matched') continue
    if (
      observation.status === 'missing' &&
      parentDirectories(entry.path).some(
        (parent) => observedByPath.get(parent)?.status !== 'matched',
      )
    ) {
      continue
    }
...
interface LayoutScanPolicy {
  readonly declared: ReadonlySet<string>
  readonly requiredDirectories: ReadonlySet<string>
  readonly ignored: readonly RegExp[]
}

async function scanPhysicalPath(
  catalogRoot: string,
  moduleRoot: string,
  path: string,
  output: Map<string, LayoutObservedKind>,
  diagnostics: Diagnostic[],
  policy: LayoutScanPolicy,
): Promise<void> {
  ...
  const kind: LayoutObservedKind = metadata.isSymbolicLink()
    ? 'symbolic-link'
  ...
  const observedPath = relativePath
    ? kind === 'directory'
      ...
      : relativePath
    : './'
  if (ignoredObservedPath(observedPath, kind, policy)) return
  output.set(observedPath, kind)
  if (kind !== 'directory') return
  if (relativePath && (await isNestedModuleRoot(target))) return
  let children
  try {
  ...
  for (const child of children) {
    if (ignoredPhysicalChild(relativePath, child.name)) continue
    await scanPhysicalPath(
      catalogRoot,
      moduleRoot,
...
function layoutConfiguration(
  ...
  | {
      readonly entries?: ts.ArrayLiteralExpression
      readonly exact: boolean
  ...
  for (const member of object.properties) {
    ...
    if (name === 'entries' || name === 'exact' || name === 'ignore') continue
    diagnostics.push(
      nodeDiagnostic(
        'LAYOUT_OPTION_INVALID',
        'Configured layouts may contain only entries, exact, and ignore.',
        source,
        file,
        member,
      ),
  ...
  if (!entries || !ts.isArrayLiteralExpression(entries.initializer)) {
    diagnostics.push(
      nodeDiagnostic(
        'LAYOUT_ENTRIES_INVALID',
        'Configured layouts require one literal entries array.',
        source,
        file,
        entries ?? object,
      ),
  ...
  if (exact) {
    if (exact.initializer.kind === ts.SyntaxKind.TrueKeyword) exactValue = true
    else if (exact.initializer.kind !== ts.SyntaxKind.FalseKeyword) {
      diagnostics.push(
        nodeDiagnostic(
          'LAYOUT_EXACT_INVALID',
          'Layout exact must be a boolean literal.',
          source,
          file,
          exact.initializer,
        ),
  ...
  if (ignore) {
    if (!ts.isArrayLiteralExpression(ignore.initializer)) {
      diagnostics.push(
        nodeDiagnostic(
          'LAYOUT_IGNORE_INVALID',
          'Layout ignore must be an array of relative POSIX glob string literals.',
          source,
          file,
          ignore.initializer,
        ),
      ...
      for (const element of ignore.initializer.elements) {
        ...
        if (!pattern || !validIgnorePattern(pattern)) {
          diagnostics.push(
            nodeDiagnostic(
              'LAYOUT_IGNORE_PATTERN_INVALID',
              'Ignore patterns must be canonical relative POSIX globs using *, **, or ?.',
              source,
              file,
              element,
            ),
        ...
        if (seen.has(pattern)) {
          diagnostics.push(
            nodeDiagnostic(
              'LAYOUT_IGNORE_PATTERN_DUPLICATE',
              `Layout ignore pattern ${pattern} is duplicated.`,
              source,
              file,
              element,
            ),
...
function layoutLiteral(
  expression: ts.Expression,
  helper: string | undefined,
): ts.ArrayLiteralExpression | ts.ObjectLiteralExpression | undefined {
  if (
    !helper ||
...
function effectiveIgnorePatterns(authored: readonly string[]): LayoutIgnorePattern[] {
  const patterns = new Map<string, LayoutIgnorePattern>()
  for (const pattern of DEFAULT_LAYOUT_IGNORE_PATTERNS) {
    patterns.set(pattern, { pattern, source: 'default' })
  }
  for (const pattern of authored) patterns.set(pattern, { pattern, source: 'layout' })
  return [...patterns.values()]
}

function ignoredObservedPath(
  path: string,
  kind: LayoutObservedKind,
  policy: LayoutScanPolicy,
): boolean {
  const stem = pathStem(path)
  const declared =
    policy.declared.has(stem) || (kind === 'directory' && policy.requiredDirectories.has(stem))
...
async function isNestedModuleRoot(directory: string): Promise<boolean> {
  try {
...
function ignoredPhysicalChild(parent: string, name: string): boolean {
  if (
    [
      '.context',
      '.history',
  ...
  if (
    ((insideBenchmark || insideQualification) &&
...
function layoutPathKind(path: string): LayoutEntry['kind'] | undefined {
  if (
    !path ||
    path.trim() !== path ||
    isAbsolute(path) ||
  ...
  if (!stem || segments.some((segment) => !segment || segment === '.' || segment === '..')) return
  return path.endsWith('/') ? 'directory' : 'file'
}

function selfSpecifyingPath(path: string): boolean {
...
function article(kind: LayoutObservedKind): 'a' | 'an' {
  return kind === 'other' ? 'an' : 'a'
}

function isLayoutControl(code: number): boolean {
