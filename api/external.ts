...
interface ExternalNode {
  ...
  heritage: boolean
  member: boolean
  callableMember: boolean
  typeArity?: number
  value: boolean
  readonly children: Map<string, ExternalNode>
}
 ...
 */
export function collectExternalReferences(source: ts.SourceFile): readonly ExternalReference[] {
  ...
  for (const statement of source.statements) {
    if (ts.isImportDeclaration(statement) && ts.isStringLiteral(statement.moduleSpecifier)) {
      ...
      const specifier = statement.moduleSpecifier.text
      if (isExternalSpecifier(specifier)) collectExport(statement, specifier, references)
    } else if (
  ...
  if (!bindings.size && !/\bimport\s*\(/u.test(source.text)) return deduplicate(references)

  const visit = (node: ts.Node): void => {
    if (ts.isTypeReferenceNode(node)) {
      collectBoundReference(
        entityPath(node.typeName),
        'type',
      ...
      if (owner && member) {
        collectBoundReference(
          [...owner, member],
          callableIndexedAccess(node) ? 'callable-member' : 'member',
          0,
          bindings,
          references,
        )
      ...
      collectBoundReference(
        expressionPath(node.expression),
        'heritage',
        node.typeArguments?.length ?? 0,
        bindings,
        references,
...
export function renderExternalModules(
  groups: readonly (readonly ExternalReference[])[],
): ReadonlyMap<string, string> {
  const modules = new Map<string, ExternalNode>()
  for (const references of groups) {
    for (const reference of references) {
      ...
      if (reference.kind === 'module' && reference.path.length === 0) continue
      const node = nodeAt(root, reference.path)
      if (reference.kind === 'import') node.imported = true
      else if (reference.kind === 'type' || reference.kind === 'heritage') {
        node.typeArity = Math.max(node.typeArity ?? 0, reference.arity)
        if (reference.kind === 'heritage') node.heritage = true
      } else if (reference.kind === 'member' || reference.kind === 'callable-member') {
        node.member = true
        if (reference.kind === 'callable-member') node.callableMember = true
      } else if (reference.kind === 'value') node.value = true
    }
  }
  return new Map(
    [...modules]
...
function collectImport(
  ...
  if (!clause) throw new Error(`External API imports must bind types: ${specifier}`)
  references.push(moduleReference(specifier))

  if (clause.name) {
    if (!clause.isTypeOnly) throw new Error(`External API imports must be type-only: ${specifier}`)
    const path = ['default']
...
function collectBoundReference(
  path: readonly string[] | undefined,
  kind: 'type' | 'heritage' | 'member' | 'callable-member' | 'value',
  arity: number,
  bindings: ReadonlyMap<string, ExternalBinding>,
  references: ExternalReference[],
): void {
...
function collectImportType(node: ts.ImportTypeNode, references: ExternalReference[]): void {
...
function moduleReference(specifier: string): ExternalReference {
  return { specifier, path: [], kind: 'module', arity: 0 }
}

function entityPath(name: ts.EntityName): readonly string[] {
  const path: string[] = []
  let current = name
...
function typeReferencePath(node: ts.TypeNode): readonly string[] | undefined {
  return ts.isTypeReferenceNode(node) ? entityPath(node.typeName) : undefined
}

function indexedAccessMember(node: ts.TypeNode): string | undefined {
  return ts.isLiteralTypeNode(node) && ts.isStringLiteral(node.literal)
    ? node.literal.text
    : undefined
}

function callableIndexedAccess(node: ts.IndexedAccessTypeNode): boolean {
...
function deduplicate(references: readonly ExternalReference[]): ExternalReference[] {
  const values = new Map<string, ExternalReference>()
  for (const reference of references) {
    const key = `${reference.specifier}\0${reference.path.join('.')}\0${reference.kind}`
    const current = values.get(key)
    if (!current || reference.arity > current.arity) values.set(key, reference)
  }
...
function externalNode(): ExternalNode {
  return {
    imported: false,
    heritage: false,
    member: false,
...
function nodeAt(root: ExternalNode, path: readonly string[]): ExternalNode {
  let current = root
...
function renderModule(root: ExternalNode): string {
  ...
  if (defaultNode) declarations.push(...renderDefault(root, defaultNode))
  if (!declarations.length) declarations.push('export {}')
  return `${declarations.join('\n')}\n`
}

function renderDefault(root: ExternalNode, node: ExternalNode): string[] {
  let local = '__AstraleExternalDefault'
  while (root.children.has(local)) local += '_'
...
function renderNamed(
  ...
  if (node.member) {
    declarations.push(
      `${indent}${prefix}interface ${name}${typeParameters(node.typeArity ?? 0)} {}`,
    )
  } else if (node.imported && node.value) {
    declarations.push(
      `${indent}${prefix}declare class ${name}${typeParameters(node.typeArity ?? 0)} {}`,
    )
    if (node.children.size) {
  ...
  if (node.children.size) {
    ...
    if (namespaceChildren.length) {
      declarations.push(`${indent}${prefix}namespace ${name} {`)
      declarations.push(
        ...namespaceChildren.flatMap(([childName, child]) =>
          renderNamed(childName, child, `${indent}  `, true),
        ),
      )
      declarations.push(`${indent}}`)
    }
  ...
  if (node.typeArity !== undefined && node.value) {
    ...
    declarations.push(
      `${indent}${prefix}type ${name}${typeParameters(node.typeArity ?? 0)} = unknown`,
    )
  } else if (node.value) {
    declarations.push(`${indent}${prefix}declare const ${name}: unknown`)
  }
...
function renderChildren(node: ExternalNode, indent: string): string[] {
  return [...node.children]
    .sort(([left], [right]) => compare(left, right))
    .flatMap(([name, child]) => renderNamed(name, child, indent, true))
}

function typeParameters(arity: number): string {
  if (!arity) return ''
