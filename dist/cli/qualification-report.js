import { printDiagnostic, terminalText } from './report.js';
export function qualificationDiagnostics(qualification) {
    return qualification.profiles.flatMap((profile) => profile.rules.flatMap((rule) => rule.diagnostics.map((diagnostic) => sourceDiagnostic(qualification, diagnostic))));
}
export function printQualificationProfile(output, source, profile) {
...
export function printQualificationRule(output, qualification, rule) {
    if (rule.status === 'pass' && !rule.diagnostics.length)
        return;
    if (!rule.diagnostics.length) {
        output.error(`${terminalText(qualification.specification.source)} [${terminalText(rule.rule)}] ${rule.status}`);
        return;
    }
    for (const diagnostic of rule.diagnostics) {
        ...
        if (diagnostic.hint)
            output.error(`  hint: ${terminalText(diagnostic.hint)}`);
    }
}
/** Print one bounded failure overview while full rule evidence remains behind --details. */
export function printQualificationSummary(output, qualification) {
    if (qualification.status === 'pass')
        return;
    const failing = qualification.profiles.filter((profile) => profile.status !== 'pass');
    const diagnostics = failing.flatMap((profile) => profile.rules.flatMap((rule) => rule.diagnostics));
    output.error(`${terminalText(qualification.specification.source)} ${qualification.status.toUpperCase()} — ${diagnostics.length} diagnostic${diagnostics.length === 1 ? '' : 's'} in ${failing.length} profile${failing.length === 1 ? '' : 's'}`);
    for (const profile of failing) {
        output.error(`  ${terminalText(profile.id)} ${profile.status} forward=${profile.coverage.forward.matched}/${profile.coverage.forward.total} inverse=${profile.coverage.inverse.matched}/${profile.coverage.inverse.total}`);
        ...
        for (const group of groups) {
            ...
            output.error(`    ${terminalText(group.code)} ×${group.count} — ${terminalText(diagnostic.file)}:${diagnostic.line}:${diagnostic.column} ${terminalText(group.diagnostic.message)}`);
        }
    }
}
function sourceDiagnostic(qualification, diagnostic) {
...
function diagnosticsFor(profile) {
...
function grouped(values) {
    ...
    for (const diagnostic of values) {
        ...
        if (current)
            ...
            groups.set(diagnostic.code, { code: diagnostic.code, count: 1, diagnostic });
    }
