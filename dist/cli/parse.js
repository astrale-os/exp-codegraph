...
export function parseCommand(input, environment = process.env) {
    const args = [...input];
    ...
    if (command === 'changed')
        return { name: 'changed', ...parseChanged(args, cache) };
    if (command === 'init')
        return { name: 'init', root: parseSingleRoot(args) };
    if (command === 'test')
        return { name: 'test', ...parseTest(args, cache) };
    if (command === 'verify')
        return { name: 'verify', ...parseVerify(args) };
    if (command === 'dev')
        return { name: 'dev', ...parseDev(args, cache) };
    return usageError();
}
function parseTest(args, cacheDefault) {
    let root = '.';
    let quiet = false;
...
function parseChanged(args, cacheDefault) {
    let root = '.';
    let base;
    let hasRoot = false;
    const exclude = [];
...
function parseSingleRoot(args) {
    if (args.length > 1 || args[0]?.startsWith('-'))
        usageError();
    return resolve(args[0] ?? '.');
}
function parseCheck(args, cacheDefault) {
    let root = '.';
    let hasRoot = false;
    const exclude = [];
    const select = [];
    ...
    for (let index = 0; index < args.length; index++) {
        ...
        else if (argument.startsWith('-') || hasRoot)
            usageError();
        else {
            root = argument;
            hasRoot = true;
...
function parseVerify(args) {
    ...
    for (let index = 0; index < args.length; index++) {
        const argument = args[index];
        if (argument === '--require-pass' && !requirePass)
            requirePass = true;
        else if (argument === '--details' && !details)
            details = true;
        else if (argument === '--quiet' && !quiet)
            quiet = true;
        else if (argument === '--select') {
...
function parseDev(args, cacheDefault) {
    let root = '.';
    let hasRoot = false;
    let port;
    let open = false;
...
function persistentCacheByDefault(environment) {
