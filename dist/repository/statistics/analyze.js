...
import { analyzeSourceLines, defaultRepositorySourceLineAnalyzers, emptySourceLines } from './lines.js';
/** Analyze one immutable inventory without creating a compiler project or retaining source text. */
export async function analyzeRepositoryStatistics(options) {
...
function unavailableFile(file, issue) {
    const failure = {
        code: issue.code,
        message: issue.message,
        retryable: issue.code !== 'REPOSITORY_STATISTICS_ANALYZER_FAILED',
    };
    const completeness = { kind: 'unavailable', reasons: [failure] };
    return {
        ...file,
        lines: emptySourceLines(),
