import type { Completeness, FactShard } from '../analysis/facts/index.ts';
import type { AnalysisGenerationId, PassId, RepositoryId, SourceId, SourceManifestId, SourceRevisionId } from '../analysis/identity/index.ts';
export type RepositoryPurpose = 'implementation' | 'test' | 'test-support' | 'fixture' | 'specification' | 'evidence' | 'unknown';
...
export interface RepositoryScanner {
    ...
    scanAll?(root: string, options?: {
        readonly signal?: AbortSignal;
        readonly scope?: RepositoryScope;
    }): Promise<readonly RepositoryScanEntry[]>;
    scan(root: string, options?: {
        readonly signal?: AbortSignal;
        readonly scope?: RepositoryScope;
...
export declare function inventoryRepository(options: RepositoryInventoryOptions): Promise<RepositoryInventory>;
export declare function repositoryFacts(inventory: RepositoryInventory, context: {
    readonly generation: AnalysisGenerationId;
    readonly pass: PassId;
    readonly passVersion: string;
}): readonly FactShard[];
export declare function createNodeRepositoryScanner(): RepositoryScanner;
export declare function defaultRepositoryClassifiers(): readonly RepositoryClassifier[];
export declare function repositoryDirectoryExcluded(path: string, patterns: readonly string[]): boolean;
