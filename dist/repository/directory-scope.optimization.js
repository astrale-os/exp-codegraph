...
export function simpleDirectoryExclusion(path, normalizedPattern) {
    ...
    if (!segment || GLOB_META.test(segment))
        return;
    return (path === segment ||
...
export function simpleRepositoryPathMatch(path, normalizedPattern) {
    if (!GLOB_META.test(normalizedPattern))
