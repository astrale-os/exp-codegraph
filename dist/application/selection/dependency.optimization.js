...
import { visitModuleReferences } from '../../specification/module/typescript-reference.js';
 ...
 */
export async function planApplicationDependencyOptimization(root, anchors, primary, inventory, sources, signal) {
    try {
        ...
        const bySource = new Map(anchors.map((anchor) => [anchor.source, anchor]));
        ...
        while (pending.length) {
            ...
            const admit = (target) => {
                if (!target || target.source === owner.source)
                    return;
                dependencySet(dependencies, owner.source).add(target.source);
                if (closure.has(target.source))
                    return;
            ...
            for (const file of sourcesByOwner.get(owner.source) ?? []) {
                ...
                const parsed = ts.createSourceFile(sourceFile, read.text, ts.ScriptTarget.ES2022, false, ts.ScriptKind.TS);
...
function specificationSourcesByOwner(inventory, owners) {
    const values = new Map();
    for (const file of inventory.files) {
        if (!specificationTypeScript(file.path, file.content))
            continue;
...
function specificationOwnerIndex(anchors) {
    return {
        anchors: [...anchors].sort((left, right) => dirname(right.source).length - dirname(left.source).length || compareSource(left, right)),
        bySource: new Map(anchors.map((anchor) => [anchor.source, anchor])),
    };
}
function ownerForPath(path, owners) {
...
function importedOwner(root, sourceFile, specifier, owners) {
...
function referencedOwner(root, sourceFile, specifier, owners) {
    return ownerForAbsolute(root, resolve(dirname(sourceFile), specifier), owners);
}
function typeReferenceOwner(root, sourceFile, specifier, owners) {
    const resolved = ts.resolveTypeReferenceDirective(specifier, sourceFile, COMPILER_OPTIONS, ts.sys).resolvedTypeReferenceDirective?.resolvedFileName;
    return resolved ? ownerForAbsolute(root, resolved, owners) : undefined;
}
function ownerForAbsolute(root, absolute, owners, mode = 'containing-owner') {
