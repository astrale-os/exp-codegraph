...
export function selectApplicationSpecifications(root, specifications, options = {}) {
    ...
    const dependencies = new Map(specifications.map((specification) => [
        specification.source,
        new Set(specification.sourceReferences
    ...
    if (options.includeDependents) {
        ...
        while (changed) {
            ...
            for (const [source, imports] of dependencies) {
                if (selected.has(source) || ![...imports].some((dependency) => selected.has(dependency)))
                    continue;
                selected.add(source);
                changed = true;
            }
...
function invalidSelection(specifications, options, code, error, requested = []) {
