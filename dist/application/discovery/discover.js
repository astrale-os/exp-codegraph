...
export async function resolveApplicationRoot(input) {
...
export async function discoverSpecificationDirectories(directory, options = {}) {
    const excluded = excludedDirectories(directory, options.exclude ?? []);
    return discover(directory, excluded);
}
async function discover(directory, excluded) {
...
function skipDirectory(parent, name) {
    if (SKIP.has(name) || (name.startsWith('.') && name !== '.spec'))
        return true;
    const parentName = basename(parent);
    return ((parentName === 'qualification' && name === 'evidence') ||
        (name === 'artifacts' && SNAPSHOT_ARTIFACT_PARENTS.has(parentName)));
}
function excludedDirectories(root, inputs) {
...
function isExcluded(directory, excluded) {
    if (!excluded.length)
        return false;
