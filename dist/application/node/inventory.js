...
import { seedOperationSourceText } from '../../source/operation-snapshot.js';
import { TYPE_SPEC_APPLICATION_LIMITS } from '../limits.js';
import { inventoryGitHead } from './git-inventory.optimization.js';
import { captureGitExecutable, createGitSourceProofProvider } from './source-proof.js';
import { repositoryDirectoryExcluded, repositoryDirectoryTopologyFingerprint, } from './topology.js';
export { repositoryDirectoryTopologyFingerprint } from './topology.js';
...
export function createGitRepositoryInventory(options) {
    ...
    return async (request) => {
        ...
        const stagedInventory = inventoryGitHead(git, root, request, inventory).then((value) => ({ ok: true, value }), (error) => ({ ok: false, error }));
        try {
            const proofStarted = performance.now();
            const admitted = await proof.admit(root, {
...
function inventoryWork(inventory) {
...
function observeDecision(options, decision) {
    try {
...
async function scanMetadata(root, relative, exclude, directories, signal) {
    signal?.throwIfAborted();
