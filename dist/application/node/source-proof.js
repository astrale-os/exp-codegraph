...
import { createSourceProof } from '../../repository/index.js';
import { repositoryDirectoryExcluded, repositoryDirectoryTopologyFingerprint, } from './topology.js';
...
export function createGitSourceProofProvider() {
    const git = captureGitExecutable();
    return Object.freeze({
        async admit(root, scope, signal) {
            const first = await admitGitSourceProof(git, root, scope, signal);
            if (first.ok || first.code !== 'proof-unstable')
                return first;
            return admitGitSourceProof(git, root, scope, signal);
        },
    });
}
async function admitGitSourceProof(git, inputRoot, scope, signal) {
    try {
        signal?.throwIfAborted();
        const root = await realpath(resolve(inputRoot));
        const identity = lines(await git.run(root, [
            'rev-parse',
        ...
        const [repositoryFormatBytes, sparseBytes, status, ignored, topologyDigest] = await Promise.all([
            git.run(root, ['config', '--type=int', '--default=0', '--get', 'core.repositoryformatversion'], signal),
            git.run(root, ['config', '--type=bool', '--default=false', '--get', 'core.sparseCheckout'], signal),
            git.run(root, [
                'status',
                '--porcelain=v2',
                '-z',
            ...
            git.run(root, ['ls-files', '--others', '-i', '--exclude-standard', '--directory', '-z'], signal),
        ...
        if (semanticIgnored.length) {
            return fallback('proof-unsupported', `Semantic source paths are ignored by Git: ${semanticIgnored.slice(0, 3).join(', ')}`);
        }
        ...
        if (unreadable(error)) {
            return fallback('proof-unreadable', `Git source proof cannot read a semantic input: ${boundedMessage(error)}`);
        }
        return fallback('proof-unsupported', `Git source proof is unavailable: ${boundedMessage(error)}`);
    }
}
function parseStatus(bytes, scope) {
...
function rejectedStatus(code, message) {
    return { ok: false, admission: fallback(code, message) };
}
function rememberCandidate(values, input, previousMode, scope) {
    const path = gitSourcePath(input);
    if (gitSourcePathIncluded(path, scope))
        values.set(path, { path, previousMode });
}
async function dirtyOverlayEntry(root, candidate, signal) {
    ...
    if (before.isSymbolicLink()) {
        ...
        if (!sameMetadata(before, after)) {
            throw new SourceProofError('proof-unstable', `Dirty source changed during admission: ${candidate.path}`);
        }
        return {
            path: candidate.path,
            kind: 'content',
    ...
    if (!before.isFile()) {
        throw new SourceProofError('proof-unsupported', `Dirty source has an unsupported type: ${candidate.path}`);
    }
    const handle = await open(file, 'r');
    try {
        const opened = await handle.stat({ bigint: true });
        if (!sameMetadata(before, opened)) {
            throw new SourceProofError('proof-unstable', `Dirty source changed before admission: ${candidate.path}`);
        }
...
export function captureGitExecutable() {
    const execute = spawn;
    return Object.freeze({
        run(root, args, signal, input) {
            return new Promise((resolveResult, reject) => {
                ...
                child.stdout.on('data', (chunk) => {
                    ...
                    if (stdoutBytes > MAXIMUM_GIT_OUTPUT_BYTES)
                        abortWithError('Git output exceeded its bound.');
                    else
                        stdout.push(chunk);
                });
                child.stderr.on('data', (chunk) => {
                    stderrBytes += chunk.byteLength;
                    if (stderrBytes > MAXIMUM_GIT_ERROR_BYTES)
                        abortWithError('Git error output exceeded its bound.');
                    else
                        stderr.push(chunk);
                });
                ...
                child.on('close', (code, childSignal) => {
                    if (settled)
                        return;
                    if (code === 0)
                        finish(() => resolveResult(Buffer.concat(stdout, stdoutBytes)));
                    else {
                        const detail = Buffer.concat(stderr, stderrBytes).toString('utf8').trim();
                        finish(() => reject(new Error(`git exited with ${childSignal ?? code}: ${detail || 'no diagnostic'}`)));
                    }
                });
...
function fixedRecord(record, fields) {
    const values = [];
    let start = 0;
...
export function gitSourcePath(input) {
...
function fallback(code, message, retryable = false) {
...
function unreadable(error) {
    if (error && typeof error === 'object' && 'code' in error &&
        ...
        return true;
    return /permission denied|operation not permitted/iu.test(error instanceof Error ? error.message : String(error));
}
