...
import { createBoundedFileCacheStore, defaultTypeSpecCacheDirectory, } from '../../cache/file-store.js';
...
const ROOT_FILES = ['cli.ts', 'index.ts', 'native-release.json', 'package.json'];
...
async function persistentFingerprint(packageRoot, mode) {
    ...
    if (cached) {
        try {
            const value = JSON.parse(cached.toString('utf8'));
            if (value.format === FINGERPRINT_FORMAT &&
                value.metadata === metadata &&
                typeof value.fingerprint === 'string' &&
                /^@astrale-os\/codegraph:codegraph-runtime-tree\/1:sha256:[a-f0-9]{64}$/u.test(value.fingerprint)) {
    ...
    await store.save(Buffer.from(JSON.stringify({ format: FINGERPRINT_FORMAT, metadata, fingerprint: value }), 'utf8'));
    return value;
}
async function fingerprint(packageRoot, mode) {
    const files = await fingerprintFiles(packageRoot, mode);
    return fingerprintFilesContent(packageRoot, files);
}
async function fingerprintFiles(packageRoot, mode) {
    ...
    if (!files.length)
        throw new Error(`Codegraph runtime tree is empty: ${packageRoot}`);
    return files;
}
async function fingerprintFilesContent(packageRoot, files) {
...
async function metadataFingerprint(packageRoot, files) {
...
async function runtimeFiles(path) {
    let metadata;
    try {
...
function defaultPackageCoordinates() {
