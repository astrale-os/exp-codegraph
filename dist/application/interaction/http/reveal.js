import { SPEC_REVEAL_ENDPOINT, SPEC_REVEAL_HEADER, SPEC_REVEAL_PROTOCOL } from '../reveal.js';
export async function handleSpecRevealHttp(request, response, execute) {
    if (!request.url)
        return false;
    const url = new URL(request.url, 'http://127.0.0.1');
    if (url.pathname !== SPEC_REVEAL_ENDPOINT)
        return false;
    ...
    const unsupported = [...url.searchParams.keys()].some((key) => key !== 'source' && key !== 'snapshot');
    if (sources.length !== 1 ||
        unsupported ||
        sources[0].length === 0 ||
    ...
    try {
        const result = await execute(sources[0], snapshots[0]);
