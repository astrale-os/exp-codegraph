...
const FLAT_SHARDS = new WeakSet();
const FLAT_REFERENCES = new WeakSet();
...
export function matchesMaterializedManifest(shards, transaction) {
    const original = () => {
    ...
    if ('toJSON' in Object.prototype || 'toJSON' in Array.prototype)
        return original();
    ...
    if (types.isProxy(transaction))
        return original();
    const manifestField = Object.getOwnPropertyDescriptor(transaction, 'manifest');
    if (!manifestField || !('value' in manifestField) || !plainArray(manifestField.value))
        return original();
    const manifest = manifestField.value;
    for (const [key, shard] of shards) {
        if (!flatShard(shard) || shard.key !== key)
            return original();
    }
    ...
    for (let index = 0; index < manifest.length; index++) {
        ...
        if (!field || !('value' in field) || !flatReference(field.value))
            return original();
        ...
        if (previous !== undefined && reference.key.localeCompare(previous) <= 0)
            matches = false;
        ...
        if (!shard || reference.digest !== shard.digest || reference.namespace !== shard.namespace ||
            ...
            !sameCapabilities(reference.capabilities, shard.capabilities))
            matches = false;
    }
    return matches;
}
function flatShard(shard) {
    if (FLAT_SHARDS.has(shard))
        return ownedOptionalCapabilities(shard);
    if (!plainRecord(shard))
        return false;
    for (const [key, type] of [['key', 'string'], ['digest', 'string'], ['namespace', 'string'], ['schemaVersion', 'number']]) {
        const descriptor = Object.getOwnPropertyDescriptor(shard, key);
        if (!descriptor || !('value' in descriptor) || typeof descriptor.value !== type ||
            type === 'number' && !Number.isFinite(descriptor.value))
            return false;
    }
    ...
    if (!facts || !('value' in facts) || !Array.isArray(facts.value) || types.isProxy(facts.value) ||
        capabilities && (!('value' in capabilities) || capabilities.value !== undefined && !stringArray(capabilities.value)))
        return false;
    // Inherited fields would be read by shardReference but are not owned data.
    if (!capabilities && 'capabilities' in shard)
        return false;
...
function flatReference(value) {
    if (!value || typeof value !== 'object')
        return false;
    if (FLAT_REFERENCES.has(value))
        return ownedOptionalCapabilities(value);
    if (!plainRecord(value))
        return false;
    for (const key of Object.keys(value))
        if (!REFERENCE_KEYS.has(key))
            return false;
    for (const [key, type] of [['key', 'string'], ['digest', 'string'], ['namespace', 'string'], ['schemaVersion', 'number'], ['facts', 'number']]) {
    ...
    if (capabilities && (!capabilities.enumerable || !('value' in capabilities) ||
        capabilities.value !== undefined && !stringArray(capabilities.value)))
        return false;
    if (!capabilities && 'capabilities' in value)
        return false;
    if (Object.isFrozen(value) && (!capabilities || capabilities.value === undefined || Object.isFrozen(capabilities.value))) {
        FLAT_REFERENCES.add(value);
    }
    return true;
}
...
function ownedOptionalCapabilities(value) {
