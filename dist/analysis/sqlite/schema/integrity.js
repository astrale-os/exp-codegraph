...
import { encodeJson, generationFromRow } from '../materialization/model.js';
import { loadManifest, loadMaterializedGeneration } from '../materialization/read.js';
export function verifySQLiteAnalysisIntegrity(database, storeNamespace, payloadCodecs, maximumDecompressedShardPayloadBytes) {
    ...
    for (const row of generations) {
        ...
        try {
            ...
            const manifest = loadManifest(database, storeNamespace, generation.universe, generation.sequence);
            manifestJson = encodeJson(manifest);
            const materialized = loadMaterializedGeneration(database, storeNamespace, generation, payloadCodecs, maximumDecompressedShardPayloadBytes);
            ...
            if (diagnostics.length)
                throw new Error([...new Set(diagnostics)].sort().join(', '));
        }
        catch (error) {
            invalid.push({
                row,
    ...
    try {
        const insert = database.prepare(`INSERT INTO analysis_quarantine
       ...
       WHERE store_namespace = ? AND universe = ? AND sequence = ?`);
        for (const entry of invalid) {
            insert.run(randomUUID(), storeNamespace, entry.row.universe, entry.row.generation_id, entry.row.sequence, entry.generationJson, entry.manifestJson, entry.reason, Date.now());
...
export function deleteOrphanedShards(database, storeNamespace) {
    database
        .prepare(`DELETE FROM analysis_shards
       WHERE store_namespace = ?
         AND NOT EXISTS (
           SELECT 1
