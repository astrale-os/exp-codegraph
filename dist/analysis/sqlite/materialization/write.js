...
import { encodeJson } from './model.js';
import { encodeShardPayloads, SQLITE_INLINE_PAYLOAD_LAYOUT, SQLITE_SHARD_PAYLOAD_LAYOUT, } from './payload.js';
export function writeTransaction(database, storeNamespace, transaction, payloads) {
    ...
    for (const shard of transaction.upserts) {
        ...
        if (!payload)
            throw new Error(`Prepared payload is unavailable for shard ${shard.digest}.`);
        writeShard(database, storeNamespace, shard, payload);
    }
    const membership = database.prepare(`INSERT INTO analysis_generation_shards
      (store_namespace, universe, generation_sequence, shard_key, shard_digest)
     VALUES (?, ?, ?, ?, ?)`);
    for (const entry of transaction.manifest) {
        membership.run(storeNamespace, transaction.next.universe, transaction.next.sequence, entry.key, entry.digest);
    }
    database
        .prepare(`INSERT INTO analysis_current
...
export function writeMaterializedGeneration(database, storeNamespace, materialized) {
    writeGeneration(database, storeNamespace, materialized.generation);
    const membership = database.prepare(`INSERT INTO analysis_generation_shards
      (store_namespace, universe, generation_sequence, shard_key, shard_digest)
     VALUES (?, ?, ?, ?, ?)`);
    for (const shard of materialized.shards.values()) {
        writeShard(database, storeNamespace, shard, encodeShardPayloads(shard.facts, DEFAULT_SQLITE_ANALYSIS_LIMITS.maximumDecompressedShardPayloadBytes));
        membership.run(storeNamespace, materialized.generation.universe, materialized.generation.sequence, shard.key, shard.digest);
    }
}
function writeGeneration(database, storeNamespace, generation) {
    database
        .prepare(`INSERT INTO analysis_generations
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(storeNamespace, generation.universe, generation.sequence, generation.id, generation.producer.id, generation.producer.name, generation.producer.version, generation.producer.protocolVersion, generation.sourceManifest, encodeJson(generation.capabilities));
}
function writeShard(database, storeNamespace, shard, preparedPayloads) {
    const payloadLayout = preparedPayloads.layout;
    const existing = database
        .prepare(`SELECT * FROM analysis_shards
       WHERE store_namespace = ? AND shard_digest = ?`)
        .get(storeNamespace, shard.digest);
    if (existing) {
        if (existing.shard_key !== shard.key ||
    ...
    database
        .prepare(`INSERT INTO analysis_shards
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(storeNamespace, shard.digest, shard.key, shard.namespace, shard.schemaVersion, shard.completion.kind, encodeJson(shard.completion), encodeJson(shard.capabilities ?? []), shard.facts.length, payloadLayout);
    const factStatement = database.prepare(`INSERT INTO analysis_facts
    ...
    for (const [payloadOrdinal, fact] of shard.facts.entries()) {
        const payloadJson = payloadLayout === SQLITE_INLINE_PAYLOAD_LAYOUT
            ? preparedPayloads.payloads[payloadOrdinal]
            : encodeJson(payloadOrdinal);
        if (payloadJson === undefined) {
            throw new Error(`Prepared inline payload is unavailable for fact ${fact.id}.`);
        }
        factStatement.run(storeNamespace, shard.digest, fact.id, fact.namespace, fact.schemaVersion, fact.kind, fact.subject, fact.completeness.kind, encodeJson(fact.completeness), fact.provenance.pass, fact.provenance.passVersion, payloadJson);
        fact.provenance.evidence.forEach((evidence, ordinal) => {
            evidenceStatement.run(storeNamespace, shard.digest, fact.id, ordinal, evidence.source, evidence.revision, evidence.start, evidence.end);
        });
