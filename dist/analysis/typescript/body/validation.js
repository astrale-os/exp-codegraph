...
export function validateBodyView(body, identities) {
    const diagnostics = [];
    if (!body.function)
    ...
    const occurrences = new Set(identities?.occurrences ?? body.occurrences.map((occurrence) => occurrence.id));
    ...
    const blocks = new Set(identities?.blocks ?? body.blocks.map((block) => block.id));
