...
import { admitNativeArtifactPackageManifest, currentNativeAnalysisTarget, NATIVE_ARTIFACT_PACKAGES, readNativeReleaseManifest, } from './manifest.js';
...
export async function resolvePackagedNativeAnalysis(options = {}) {
    ...
    try {
        ...
        throw new NativeAnalysisDistributionError('NATIVE_ARTIFACT_INVALID', `${artifact.package} executable is missing or unreadable.`, target, { cause });
    }
    if (!within(artifactRoot, command)) {
        throw new NativeAnalysisDistributionError('NATIVE_ARTIFACT_INVALID', `${artifact.package} executable resolves outside its package.`, target);
    }
    const admitted = await admitExecutable(command, target);
    if (admitted.bytes !== artifact.bytes || admitted.sha256 !== artifact.sha256) {
        throw new NativeAnalysisDistributionError('NATIVE_ARTIFACT_DIGEST_MISMATCH', `${artifact.package} executable does not match the qualified release manifest.`, target);
    }
