import { validateBodyView } from '../body/validation.js';
import { admitCompleteness, admitValueResult, array, createOccurrenceScratch, decodeBlock, decodeCall, decodeDefinition, decodeEdge, decodeOccurrence, decodeRelation, decodeSummary, exactRecord, exactTuple, expandId, ordinal, unique, uniqueStrings, } from './body-data.js';
...
export function preparePackedBodyIdentity(input) {
    ...
    const occurrence = (value, path) => occurrenceIds[ordinal(value, occurrenceIds.length, path)];
    ...
    const relation = { parent: '', child: '', role: '' };
    ...
    const block = (value, path) => blockIds[ordinal(value, blockIds.length, path)];
    const edgeRows = new PackedNumericRows(packed.e, 4, true, 'edges');
    const edge = { from: '', to: '', kind: 'fallthrough', evidence: undefined };
    ...
    const definition = { definition: '', use: '', symbol: undefined, reaching: 'definite' };
    ...
    const targetOrigins = Array.from(calls, (value, index) => decodeCall(value, index, 6, symbols, texts, occurrenceIds.length, occurrence).targetOrigin);
    ...
    const view = {
        ...
        occurrences: rows(occurrenceRows.length, function* () {
            ...
            for (let index = 0; index < occurrenceRows.length; index++) {
                const fields = occurrenceRows.row(index, row);
                value.id = occurrenceIds[index];
                value.kind = texts[fields[1]];
                ...
                value.symbolKind = fields[8] === -1 ? undefined : texts[fields[8]];
        ...
        blocks: rows(blocks.length, function* () {
            for (let index = 0; index < blocks.length; index++) {
                ...
                yield { id: blockIds[index], occurrences: references(row[1], occurrenceIds) };
            }
        }),
        relations: rows(relationRows.length, function* () {
            const scratch = [];
            for (let index = 0; index < relationRows.length; index++) {
                const row = relationRows.row(index, scratch);
        ...
        edges: rows(edgeRows.length, function* () {
            const scratch = [];
            for (let index = 0; index < edgeRows.length; index++) {
                const row = edgeRows.row(index, scratch);
                yield { from: blockIds[row[0]], to: blockIds[row[1]], kind: texts[row[2]],
        ...
        definitions: rows(definitionRows.length, function* () {
            const scratch = [];
            for (let index = 0; index < definitionRows.length; index++) {
                const row = definitionRows.row(index, scratch);
                yield { definition: occurrenceIds[row[0]], use: occurrenceIds[row[1]], reaching: texts[row[3]] };
            }
        }),
        calls: rows(calls.length, function* () {
            for (let index = 0; index < calls.length; index++) {
                const row = calls[index];
                yield {
                    occurrence: occurrenceIds[row[0]],
                    ...(row[1] === -1 ? {} : { target: symbols[row[1]] }),
                    ...(targetOrigins[index] === undefined ? {} : { targetOrigin: targetOrigins[index] }),
                    ...(row[3] === -1 ? {} : { receiver: occurrenceIds[row[3]] }),
                    typeArguments: row[4].map(value => texts[value]),
                    arguments: references(row[5], occurrenceIds),
                    bindings: bindingViews(row[6], occurrenceIds, symbols),
                    dynamic: row[8] === 1,
                };
            }
        }),
    ...
    return { write(writer) {
            ...
            for (let index = 0; index < blocks.length; index++) {
                ...
                const row = blocks[index];
                writer.part('{"id":');
                writer.value(blockIds[index]);
                writer.part(',"occurrences":');
                writeReferences(writer, row[1], occurrenceIds);
                writer.part('}');
            }
            writer.part('],"calls":[');
            for (let index = 0; index < calls.length; index++) {
                if (index)
                    writer.part(',');
                const row = calls[index];
                writer.part('{"arguments":');
                writeReferences(writer, row[5], occurrenceIds);
                writer.part(',"bindings":[');
                const bindings = row[6];
                for (let binding = 0; binding < bindings.length; binding++) {
                    if (binding)
                        writer.part(',');
                    const fields = bindings[binding];
                    writer.part('{"argument":');
                    writer.value(occurrenceIds[fields[0]]);
                    writer.part(',"index":');
                    writer.value(fields[2]);
                    if (fields[1] !== -1) {
                        writer.part(',"parameter":');
                        writer.value(symbols[fields[1]]);
                    }
                    writer.part(',"rest":');
                    writer.value(fields[3] === 1);
                    writer.part('}');
                }
                writer.part('],"callbacks":');
                writeReferences(writer, row[7], symbols);
                writer.part(',"dynamic":');
                writer.value(row[8] === 1);
                writer.part(',"occurrence":');
                writer.value(occurrenceIds[row[0]]);
                if (row[3] !== -1) {
                    writer.part(',"receiver":');
                    writer.value(occurrenceIds[row[3]]);
                }
                if (row[2] !== -1) {
                    writer.part(',"signature":');
                    writer.value(texts[row[2]]);
                }
                if (row[1] !== -1) {
                    writer.part(',"target":');
                    writer.value(symbols[row[1]]);
                }
                if (targetOrigins[index] !== undefined) {
                    writer.part(',"targetOrigin":');
                    writer.value(targetOrigins[index]);
                }
                writer.part(',"typeArguments":');
                writeReferences(writer, row[4], texts);
                writer.part('}');
            }
            ...
            for (let index = 0; index < definitionRows.length; index++) {
                if (index)
                    writer.part(',');
                const row = definitionRows.row(index, scratch);
                writer.part('{"definition":');
                writer.value(occurrenceIds[row[0]]);
                writer.part(',"reaching":');
                writer.value(texts[row[3]]);
                if (row[2] !== -1) {
                    writer.part(',"symbol":');
                    writer.value(symbols[row[2]]);
                }
                writer.part(',"use":');
                writer.value(occurrenceIds[row[1]]);
                writer.part('}');
            }
            writer.part('],"edges":[');
            for (let index = 0; index < edgeRows.length; index++) {
                ...
                writer.part('{');
                if (row[3] !== -1) {
                    writer.part('"evidence":');
                    writer.value(occurrenceIds[row[3]]);
                    writer.part(',');
                }
                writer.part('"from":');
                writer.value(blockIds[row[0]]);
                writer.part(',"kind":');
                writer.value(texts[row[2]]);
                writer.part(',"to":');
                writer.value(blockIds[row[1]]);
                writer.part('}');
            }
            writer.part(']');
            if (execution !== undefined) {
                writer.part(',"execution":');
                writer.value(execution);
            }
            writer.part(',"function":');
            writer.value(owner);
            writer.part(',"occurrences":[');
            for (let index = 0; index < occurrenceRows.length; index++) {
                if (index)
                    writer.part(',');
                const row = occurrenceRows.row(index, scratch);
                writer.part('{"id":');
                writer.value(occurrenceIds[index]);
                writer.part(',"kind":');
                writer.value(texts[row[1]]);
                if (row[7] !== -1) {
                    writer.part(',"operator":');
                    writer.value(texts[row[7]]);
                }
                writer.part(',"owner":');
                writer.value(owner);
                if (row[9] !== -1) {
                    writer.part(',"propertyName":');
                    writer.value(texts[row[9]]);
                }
                if (row[10] !== -1) {
                    writer.part(',"propertyNamespace":');
                    writer.value(symbols[row[10]]);
                }
                writer.part(',"span":{"end":');
                writer.value(row[3]);
                writer.part(',"revision":');
                ...
                if (row[5] !== -1) {
                    writer.part(',"symbol":');
                    writer.value(symbols[row[5]]);
                }
                if (row[8] !== -1) {
                    writer.part(',"symbolKind":');
                    writer.value(texts[row[8]]);
                }
                if (origins[index] !== undefined) {
                    writer.part(',"symbolOrigin":');
                    writer.value(origins[index]);
                }
                writer.part(',"syntax":');
                writer.value(texts[row[4]]);
                writer.part('}');
            }
            writer.part('],"parameters":');
            writer.value(parameters);
            ...
            writer.value(completeness);
            writer.part(',"values":{');
            for (let index = 0; index < values.length; index++) {
                if (index)
                    writer.part(',');
                writer.value(values[index][0]);
                writer.part(':');
                writer.value(values[index][1]);
            }
            writer.part('}}');
        } };
}
function rows(length, iterate) {
    return { length, [Symbol.iterator]: iterate };
}
function* references(indices, values) {
    for (const index of indices)
        yield values[index];
}
function* bindingViews(rows, occurrences, symbols) {
    for (const row of rows)
        yield { argument: occurrences[row[0]],
            ...(row[1] === -1 ? {} : { parameter: symbols[row[1]] }), index: row[2], rest: row[3] === 1 };
}
function writeReferences(writer, indices, values) {
    writer.part('[');
