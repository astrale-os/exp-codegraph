...
export function planPasses(manifests, requestedCapabilities, environment = {}) {
    const availableCapabilities = new Set(environment.availableCapabilities ?? []);
    const availableSchemas = new Map((environment.availableSchemas ?? []).map((schema) => [schema.namespace, schema.version]));
    ...
    for (const manifest of manifests) {
        ...
        for (const capability of manifest.providesCapabilities) {
            ...
            if (existing && existing.id !== manifest.id) {
                throw new PassPlanError('PASS_SCHEMA_INCOMPATIBLE', `Capability ${capability} has multiple providers: ${existing.id}, ${manifest.id}.`);
            }
        ...
        for (const output of manifest.outputs) {
            ...
            if (existing && existing.id !== manifest.id) {
                throw new PassPlanError('PASS_SCHEMA_INCOMPATIBLE', `Fact namespace ${output.namespace} has multiple providers.`);
            }
    ...
    const visit = (manifest) => {
        ...
        for (const capability of manifest.requiresCapabilities) {
            ...
            if (!provider) {
                throw new PassPlanError('PASS_INPUT_MISSING', `Pass ${manifest.id} requires unavailable capability ${capability}.`);
            }
            visit(provider);
        }
        for (const input of manifest.inputs) {
            ...
            if (!provider) {
                throw new PassPlanError('PASS_INPUT_MISSING', `Pass ${manifest.id} requires unavailable namespace ${input.namespace}.`);
            }
            ...
            if (output.version < input.minimumVersion || output.version > input.maximumVersion) {
                throw new PassPlanError('PASS_SCHEMA_INCOMPATIBLE', `Pass ${manifest.id} cannot read ${input.namespace}@${output.version}.`);
            }
            visit(provider);
        }
    ...
    for (const manifest of selected.values()) {
        for (const output of manifest.outputs) {
            if (availableSchemas.has(output.namespace)) {
                throw new PassPlanError('PASS_SCHEMA_INCOMPATIBLE', `Pass ${manifest.id} cannot replace base namespace ${output.namespace}.`);
            }
        }
    }
    return {
...
function validateManifest(manifest) {
    ...
    for (const input of manifest.inputs) {
        if (input.minimumVersion < 1 || input.maximumVersion < input.minimumVersion) {
            throw new PassPlanError('PASS_SCHEMA_INCOMPATIBLE', `Pass ${manifest.id} has an invalid schema range for ${input.namespace}.`);
        }
    }
}
