...
import { canonicalExpected, } from '../context.js';
import { unevaluatedRule, sameStrings, matchesExternalType, matchesExternalIdentity, withoutUndefined, memberType, expectedReadonlyShape, expectedBooleanUnion, flattenExpectedTypeSet, expectedContainsBooleanLiterals, removeSubsumedObservedLiterals, collapseObservedUnion, substituteTypeParameters, sameObservedType, intrinsicTypeCompatible, dataTypeCompatible, typeSetAssignments, mergeRule, } from '../semantics.js';
export class TypeEvaluator {
    ...
    boundObserved = new Set();
    identityCovered = new Set();
    pairs = new Set();
    observedExports = new Map();
    activeTypeAliases = new Set();
    activeShapeAliases = new Set();
    blockedDependencies = new Set();
    typeParameterScopes = new Map();
    opaqueExternalExportPaths = new Set();
    constructor(context, expected, observed) {
        ...
        for (const item of observed.exports)
            this.observedExports.set(item.path.join('.'), item);
        for (const [expectedIdentity, observedIdentity] of context.seeds) {
            this.bindings.set(expectedIdentity, observedIdentity);
        }
    }
    get identityCoveredDeclarations() {
        return this.identityCovered;
    }
    compareType(expected, observed, ruleId, location) {
        return this.compareTypeExpression(expected.expression, observed, ruleId, location, expected.pointer);
    }
    compareTypeExpression(expected, observed, ruleId, location, pointer) {
        if (observed.kind === 'unsupported') {
            this.error(ruleId, 'MODULE_TYPE_EVIDENCE_UNSUPPORTED', `Type evidence is unsupported: ${observed.reason}`, pointer, location, expected, observed);
            return false;
        }
        ...
        if (expected.kind === 'data') {
            if (dataTypeCompatible(expected.data, observed))
                return true;
        ...
        if (expectedBooleanUnion(expected) &&
        ...
        if (expected.kind === 'template') {
            ...
            return expected.types.every((item, index) => this.compareTypeExpression(item, observed.types[index], ruleId, location, pointer));
        }
        if (expected.kind === 'array') {
            ...
            return this.compareTypeExpression(expected.element, observed.element, ruleId, location, pointer);
        }
        if (expected.kind === 'record') {
            if (observed.kind !== 'record') {
                return this.typeMismatch(ruleId, pointer, location, expected, observed);
            }
            return (this.compareTypeExpression(expected.key, observed.key, ruleId, location, pointer) &&
                this.compareTypeExpression(expected.value, observed.value, ruleId, location, pointer));
        }
        if (expected.kind === 'tuple') {
            if (observed.kind !== 'tuple' ||
                (expected.readonly !== undefined && observed.readonly !== expected.readonly) ||
                observed.elements.length !== expected.elements.length) {
                return this.typeMismatch(ruleId, pointer, location, expected, observed);
            }
            return expected.elements.every((item, index) => this.compareTypeExpression(item, observed.elements[index], ruleId, location, pointer));
        }
        if (expected.kind === 'union' || expected.kind === 'intersection') {
            if (observed.kind !== expected.kind) {
                return this.typeMismatch(ruleId, pointer, location, expected, observed);
            }
            return this.compareTypeSet(expected.kind, expected.types, observed.types, ruleId, location, pointer);
        }
        if (expected.kind === 'conditional') {
            if (observed.kind !== 'conditional') {
                return this.typeMismatch(ruleId, pointer, location, expected, observed);
            }
            return (this.compareTypeExpression(expected.check, observed.check, ruleId, location, pointer) &&
                this.compareTypeExpression(expected.extends, observed.extends, ruleId, location, pointer) &&
                this.compareTypeExpression(expected.trueType, observed.trueType, ruleId, location, pointer) &&
                this.compareTypeExpression(expected.falseType, observed.falseType, ruleId, location, pointer));
        }
        ...
        if (expected.kind === 'indexed-access') {
            ...
            return (this.compareTypeExpression(expected.object, observed.object, ruleId, location, pointer) &&
        ...
        if (expected.kind === 'object') {
            ...
            for (const [member, actual] of matches) {
                ...
                if (!this.compareTypeExpression(member.expression, actualType, ruleId, location, member.pointer)) {
                    return false;
                }
            }
            return true;
        ...
        if (expected.kind === 'function' || expected.kind === 'constructor') {
            ...
            for (const [index, signature] of expectedSignatures.entries()) {
                if (!this.compareCallableType(signature, observedSignatures[index], ruleId, signature.pointer ?? pointer)) {
                    matches = false;
                }
            }
            return matches;
        ...
        if (expected.kind === 'external') {
            if (expectedReadonlyShape(expected) && observed.kind !== 'reference') {
                return this.compareTypeExpression(expected.arguments[0], observed, ruleId, location, pointer);
            }
            ...
            if (!declaration) {
                if (matchesExternalIdentity(expected, observed)) {
                    return this.compareTypeArguments(expected.arguments, observed.arguments, ruleId, location, pointer);
                }
                this.error(ruleId, 'MODULE_PUBLIC_TYPE_UNRESOLVED', `Publicly reachable type could not be observed: ${observed.name}`, pointer, location, expected, observed.identity);
                return false;
            }
            if (!matchesExternalType(expected, declaration)) {
                return this.typeMismatch(ruleId, pointer, location, expected, observed);
            }
            if (!this.compareTypeArguments(expected.arguments, this.effectiveTypeArguments(observed, declaration, expected.arguments.length), ruleId, location, pointer)) {
                return false;
            }
        ...
        if (observed.kind !== 'reference') {
            ...
            this.fail(ruleId, 'MODULE_TYPE_IDENTITY_MISMATCH', 'A named declaration reference was expected.', pointer, location, expected.declaration.key, observed);
            return false;
        }
        const actualDeclaration = this.context.observedDeclarations.get(observed.identity);
        if (!actualDeclaration) {
            this.error(ruleId, 'MODULE_PUBLIC_TYPE_UNRESOLVED', `Publicly reachable type could not be observed: ${observed.name}`, pointer, location, expected.declaration.key, observed.identity);
            return false;
        }
        ...
        const projection = actualDeclaration.kind === 'factory' &&
        ...
        if (!this.compareTypeArguments(expected.arguments, this.effectiveTypeArguments(observed, actualDeclaration, expected.arguments.length), ruleId, location, pointer)) {
            return false;
        }
        if (!wasEvaluated && !this.evaluated.has(ruleId))
            return true;
        const status = this.rulesById.get(ruleId)?.status;
        return status !== 'fail' && status !== 'error';
    }
    compareTypeArguments(expected, observed, ruleId, location, pointer) {
        if (expected.length !== observed.length) {
            return this.typeMismatch(ruleId, pointer, location, expected, observed);
        }
        return expected.every((argument, index) => this.compareTypeExpression(argument, observed[index], ruleId, location, pointer));
    }
    effectiveTypeArguments(reference, declaration, requestedLength) {
        ...
        while (arguments_.length) {
            ...
            if (!fallback ||
                !sameObservedType(arguments_[index], scope ? substituteTypeParameters(fallback, scope, arguments_) : fallback)) {
                break;
            }
            arguments_.pop();
        }
        if (arguments_.length >= requestedLength)
            return arguments_;
    ...
    bindTypeParameterScopes(expected, observed) {
    ...
    typeParameterScopeCompatible(expected, observed) {
        return (this.typeParameterScopes.get(expected) ?? expected) === observed;
    }
    matchObjectTypeMembers(expected, observed) {
        if (expected.length !== observed.length)
            return undefined;
        const used = new Set();
        const matches = [];
        const visit = (index) => {
            ...
            for (let candidate = 0; candidate < observed.length; candidate++) {
                ...
                if (used.has(candidate) ||
                    ...
                    !this.typeShapeCompatible(member.expression, actual.optional ? withoutUndefined(actualType) : actualType)) {
                    continue;
                }
                used.add(candidate);
    ...
    objectMemberSkeletonCompatible(expected, observed) {
    ...
    compareTypeSet(kind, expected, observed, ruleId, location, pointer) {
        const normalizedExpected = this.normalizeExpectedTypeSet(kind, expected);
        const normalizedObserved = this.normalizeObservedTypeSet(kind, normalizedExpected, observed);
        ...
        if (normalizedExpected.length !== normalizedObserved.length) {
            return this.typeMismatch(ruleId, pointer, location, normalizedExpected, normalizedObserved);
        }
        const candidates = normalizedExpected.map((item) => {
            const compatible = normalizedObserved.map((_, index) => index).filter((index) => this.typeShapeCompatible(item, normalizedObserved[index]));
            ...
            const identityCandidates = compatible.filter((index) => (item.kind === 'declaration' || item.kind === 'external') &&
        ...
        if (!assignments.length) {
            ...
            return this.typeMismatch(ruleId, pointer, location, unmatched >= 0 ? normalizedExpected[unmatched] : normalizedExpected, normalizedObserved);
        }
        if (assignments.length > 1) {
            this.error(ruleId, 'MODULE_TYPE_SET_AMBIGUOUS', 'Type set contains multiple indistinguishable complete matchings.', pointer, location, normalizedExpected, normalizedObserved);
            return false;
        }
        return normalizedExpected.every((item, index) => this.compareTypeExpression(item, normalizedObserved[assignments[0][index]], ruleId, location, pointer));
    }
    /**
     ...
     */
    normalizeObservedTypeSet(kind, expected, observed) {
        const output = [];
        ...
        const visit = (type) => {
            ...
            if (kind === 'union' && type.kind === 'template') {
                for (const variant of this.observedTemplateVariants(type))
                    output.push(variant);
                return;
            }
            if (type.kind === 'reference' &&
                !active.has(type.identity) &&
                !expected.some((item) => (item.kind === 'declaration' || item.kind === 'external') &&
        ...
        const expandedBooleans = expectedContainsBooleanLiterals(expected)
            ? output.flatMap((type) => type.kind === 'primitive' && type.name === 'boolean'
                ? [
     ...
     */
    coverTypeSetReferenceEvidence(expected, observed) {
        for (const item of expected) {
            ...
            if (!bound)
                continue;
            const actual = observed.find((candidate) => candidate.kind === 'reference' &&
                this.observedIdentitiesEquivalent(bound, candidate.identity) &&
                this.typeArgumentsShapeCompatible(item.arguments, this.effectiveTypeArgumentsForIdentity(candidate, item.arguments.length)));
            if (!actual || actual.kind !== 'reference')
                continue;
    ...
    markExpectedTypeDependencies(type) {
        const visit = (current) => {
            switch (current.kind) {
                case 'declaration':
                ...
                case 'union':
                case 'intersection':
                    for (const item of current.kind === 'tuple' ? current.elements : current.types)
                        visit(item);
                    return;
                case 'conditional':
                    visit(current.check);
                ...
                case 'function':
                case 'constructor':
                    for (const parameter of current.callable.parameters)
    ...
    normalizeExpectedTypeSet(kind, values) {
        const flattened = flattenExpectedTypeSet(kind, values);
        if (kind !== 'union')
            return flattened;
        return flattened.flatMap((value) => value.kind === 'template' ? this.expectedTemplateVariants(value) : [value]);
    }
    expectedTemplateVariants(template) {
        const maximumVariants = 64;
        const active = new Set();
        const expandTemplate = (current) => {
            let variants = [
        ...
        const expandHole = (type) => {
            if (type.kind === 'union')
                return type.types.flatMap(expandHole);
            if (type.kind !== 'declaration')
                return [type];
            ...
            const values = expanded.kind === 'union'
    ...
    observedTemplateVariants(template) {
        const maximumVariants = 64;
        const active = new Set();
        const expandTemplate = (current) => {
            let variants = [
        ...
        const expandHole = (type) => {
            if (type.kind === 'union')
                return type.types.flatMap(expandHole);
            if (type.kind !== 'reference' || active.has(type.identity))
                return [type];
            const expanded = this.transparentObservedType(type);
            if (!expanded ||
    ...
    typeShapeCompatible(expected, observed) {
        if (expected.kind !== 'declaration' &&
            (expected.kind !== 'external' || this.isLocalAliasReference(observed))) {
            const expanded = this.transparentObservedType(observed);
            if (expanded &&
        ...
        if (expected.kind !== 'indexed-access' && observed.kind === 'indexed-access') {
            const resolved = this.resolveObservedIndexedAccess(observed);
            if (resolved)
                return this.typeShapeCompatible(expected, resolved);
        }
        if (expected.kind === 'data') {
        ...
        if (expected.kind === 'declaration') {
            ...
            return ((seeded
                ...
                this.typeArgumentsShapeCompatible(expected.arguments, this.effectiveTypeArgumentsForIdentity(observed, expected.arguments.length)));
        }
        if (expected.kind === 'external') {
            if (expectedReadonlyShape(expected) && observed.kind !== 'reference') {
            ...
            return Boolean((declaration
                ...
                this.typeArgumentsShapeCompatible(expected.arguments, declaration
        ...
        if (expected.kind === 'template') {
            return (observed.kind === 'template' &&
                sameStrings(expected.texts, observed.texts) &&
                expected.types.length === observed.types.length &&
                expected.types.every((item, index) => this.typeShapeCompatible(item, observed.types[index])));
        }
        if (expected.kind === 'unknown')
            return observed.kind === 'unknown';
        if (expected.kind === 'parameter') {
        ...
        if (expected.kind === 'record') {
            return (observed.kind === 'record' &&
                this.typeShapeCompatible(expected.key, observed.key) &&
                this.typeShapeCompatible(expected.value, observed.value));
        }
        if (expected.kind === 'tuple') {
            return (observed.kind === 'tuple' &&
                (expected.readonly === undefined || observed.readonly === expected.readonly) &&
                observed.elements.length === expected.elements.length &&
                expected.elements.every((item, index) => this.typeShapeCompatible(item, observed.elements[index])));
        }
        if (expected.kind === 'union' || expected.kind === 'intersection') {
        ...
        if (expected.kind === 'conditional') {
            return (observed.kind === 'conditional' &&
        ...
        if (expected.kind === 'indexed-access') {
            return (observed.kind === 'indexed-access' &&
        ...
        if (expected.kind === 'function' || expected.kind === 'constructor') {
            ...
            const observedSignatures = observed.overloads?.length
                ? observed.overloads
                : [observed.callable];
            return (expectedSignatures.length === observedSignatures.length &&
                expectedSignatures.every((signature, index) => this.callableTypeShapeCompatible(signature, observedSignatures[index])));
        }
        return false;
    }
    callableTypeShapeCompatible(expected, observed) {
        ...
        return (expected.mode === observed.mode &&
            ...
            expectedTypeParameters.every((parameter, index) => {
                ...
                return (parameter.variance === actual.variance &&
            ...
            expected.parameters.every((parameter, index) => {
                ...
                return (parameter.optional === actual.optional &&
                    Boolean(parameter.rest) === actual.rest &&
                    this.typeShapeCompatible(parameter.expression, actual.optional ? withoutUndefined(actual.type) : actual.type));
            }) &&
            this.typeShapeCompatible(expected.returns.expression, observed.returns));
    }
    effectiveTypeArgumentsForIdentity(reference, requestedLength) {
    ...
    typeMismatch(ruleId, pointer, location, expected, observed) {
        this.fail(ruleId, 'MODULE_TYPE_MISMATCH', 'Public type differs from its declaration.', pointer, location, expected, observed);
        return false;
    }
    addInverse(id, status, message, location, code) {
    ...
    fail(id, code, message, pointer, actualLocation, expected, actual) {
        this.set(id, 'fail', {
            code,
            message,
    ...
    error(id, code, message, pointer, actualLocation, expected, actual) {
        this.set(id, 'error', {
            code,
            message,
            location: expectedLocation(this.expected, pointer),
