import { canonicalExpected, declarationKindCompatible, } from './context.js';
import { isTestArtifact, declarationPrefix, packagePatternMatches, safeId, } from './semantics.js';
import { DeclarationEvaluator } from './declaration/evaluator.js';
export class ModuleEvaluator extends DeclarationEvaluator {
    ...
    evaluateExports() {
        ...
        for (const item of this.expected.exports) {
            ...
            if (!declaration) {
                this.error(id, 'MODULE_EXPORT_UNRESOLVED', `TypeScript export symbol could not be materialized: ${path}`, item.pointer, actual.location);
                continue;
            }
            const expectedDeclaration = this.context.expected.declarations.get(item.declaration.key);
            const projectedTypeFacet = declaration.kind === 'factory' && actual.typeOnly && !expectedDeclaration?.facets;
            if (item.sourceModule &&
                (actual.sourceModule !== item.sourceModule || declaration.name !== item.declaration.name)) {
                this.fail(id, 'MODULE_DECLARATION_IDENTITY_MISMATCH', `External declaration identity differs for ${path}.`, item.pointer, actual.location, `${item.sourceModule}#${item.declaration.name}`, `${actual.sourceModule ?? '<local>'}#${declaration.name}`);
                continue;
            }
            if (item.sourceModule && expectedDeclaration?.conformance === 'identity') {
                if (item.typeOnly !== actual.typeOnly) {
                    this.fail(id, 'MODULE_EXPORT_TYPE_MODE_MISMATCH', `Export type/value mode differs for ${path}.`, item.pointer, actual.location, item.typeOnly ? 'type-only export' : 'runtime export', actual.typeOnly ? 'type-only export' : 'runtime export');
                    continue;
                }
                this.pass(id);
                this.opaqueExternalExportPaths.add(path);
                this.bindDeclaration(item.declaration, declaration, id, 'identity');
                this.addInverse(`observed.export.${safeId(path)}.${actual.kind}`, 'pass', `Observed export is declared: ${path}`, actual.location);
                continue;
            }
            if (declaration.kind === 'factory' &&
                ...
                !projectedTypeFacet) {
                this.error(id, 'MODULE_FACTORY_FACETS_UNSPECIFIED', `The code export ${path} has both type and runtime value facets, but api.d.ts does not declare both.`, item.pointer, actual.location);
                continue;
            }
            if (expectedDeclaration?.facets && declaration.kind !== 'factory') {
                this.fail(id, 'MODULE_FACTORY_FACETS_MISSING', `Export ${path} must provide one type-alias facet and one runtime value facet.`, item.pointer, actual.location, 'type alias + runtime value', declaration.kind);
                continue;
            }
            if (expectedDeclaration?.factory && declaration.kind !== 'factory') {
                this.fail(id, 'MODULE_FACTORY_FACETS_MISSING', `Export ${path} must provide both type and runtime value facets.`, item.pointer, actual.location, 'type + runtime value', declaration.kind);
                continue;
            }
            if (item.typeOnly !== actual.typeOnly) {
                this.fail(id, 'MODULE_EXPORT_TYPE_MODE_MISMATCH', `Export type/value mode differs for ${path}.`, item.pointer, actual.location, item.typeOnly ? 'type-only export' : 'runtime export', actual.typeOnly ? 'type-only export' : 'runtime export');
            ...
            if (!(projectedTypeFacet && item.declaration.kind === 'value') &&
                !declarationKindCompatible(item.declaration.kind, declaration, expectedDeclaration)) {
                this.fail(id, 'MODULE_EXPORT_KIND_MISMATCH', `Export kind differs for ${path}.`, item.pointer, actual.location, item.declaration.kind, actual.kind);
                continue;
            }
            if (!item.typeOnly &&
                (actual.kind === 'class' || (actual.kind === 'factory' && expectedDeclaration?.facets)) &&
                actual.typeOnly) {
                this.fail(id, 'MODULE_RUNTIME_EXPORT_MISSING', `Class export is type-only: ${path}`, item.pointer, actual.location, 'runtime class', 'type-only export');
                continue;
            }
            this.pass(id);
        ...
        for (const actual of this.observed.exports) {
            ...
            this.addInverse(`observed.export.${safeId(path)}.${actual.kind}`, 'fail', `Public export is not declared by the module specification: ${path}`, actual.location, 'MODULE_EXPORT_UNDECLARED');
        }
    }
    evaluateUnboundDeclarations() {
        for (const declaration of this.expected.declarations) {
            const id = declarationPrefix(declaration);
            if (this.evaluated.has(id))
                continue;
            this.fail(id, 'MODULE_DECLARATION_MISSING', `Specified declaration is not reachable through the public code surface: ${declaration.identity.name}`, declaration.pointer);
    ...
    evaluateErrorCodes() {
        ...
        for (const declaration of this.expected.declarations) {
            for (const code of declaration.errors ?? []) {
                ...
                this.fail(id, 'ERROR_CODE_MISSING', `Error code ${code} has no TypeScript declaration.`, `${declaration.pointer}/throws`, undefined, code, null);
            }
        }
    }
    evaluateInverseClosure() {
        for (const declaration of this.observed.declarations) {
            ...
            if (this.boundObserved.has(declaration.identity)) {
                this.addInverse(id, 'pass', `Publicly reachable type is declared: ${declaration.name}`, declaration.location);
            }
            else {
                this.evaluateUnsupportedValueShape(declaration);
                this.addInverse(id, 'fail', `Publicly reachable declaration is not represented in the specification: ${declaration.name}`, declaration.location, 'MODULE_PUBLIC_TYPE_UNDECLARED');
            }
        }
    }
    evaluateUnsupportedValueShape(declaration) {
        if (declaration.kind !== 'value')
            return;
        for (const [label, count] of [['index', declaration.indexSignatureCount ?? 0]]) {
            if (count === 0)
                continue;
            this.addInverse(`observed.shape.${safeId(declaration.identity)}.${label}`, 'error', `Public ${label} signatures are outside the supported TypeSpec declaration surface.`, declaration.location, `TYPESCRIPT_${label.toUpperCase()}_SIGNATURE_UNSUPPORTED`);
        }
    }
    evaluateImports() {
        const imported = new Set(this.expected.imports.map((identity) => canonicalExpected(identity.key, this.context.expected)));
        for (const identity of this.expected.imports) {
            ...
            if (this.usedExpected.has(canonical))
                this.pass(id);
            else
                this.fail(id, 'MODULE_CONTRACT_IMPORT_STALE', `Referenced API declaration is not used by the observed public surface: ${identity.name}`, '/api');
        }
        for (const canonical of this.usedExpected) {
            ...
            const observed = observedIdentity
                ...
                : undefined;
            this.addInverse(`observed.import.${safeId(canonical)}`, 'fail', `Public declaration dependency is absent from the authoritative API references: ${declaration.identity.name}`, observed?.location, 'MODULE_CONTRACT_IMPORT_UNDECLARED');
        }
    }
    evaluateDependencies() {
        ...
        for (const item of this.expected.packages) {
            ...
            const observed = this.observed.dependencies.some((edge) => edge.targetModule === `package:${item.name}`);
            const installed = this.observed.declaredPackages.includes(item.name);
            if (!installed) {
                this.fail(id, 'MODULE_PACKAGE_NOT_DECLARED', `Allowlisted package is absent from the code package.json: ${item.name}`, item.pointer);
            }
            else if (item.requireObserved !== false && !observed) {
                this.fail(id, 'MODULE_PACKAGE_STALE', `Allowlisted package has no observed code or public-API dependency: ${item.name}`, item.pointer);
            }
            else {
                this.pass(id);
            }
        ...
        for (const edge of this.observed.dependencies) {
            ...
            const declaredPackage = packageName
                ...
                : false;
            const declaredWorkspacePackage = Boolean(packageName &&
            ...
            const occurrenceResults = edge.occurrences.map((occurrence) => {
                const declaredTestPackage = Boolean(packageName &&
                    isTestArtifact(edge.sourceFile) &&
                    this.observed.developmentPackages.includes(packageName));
                const declaredByPublicProvider = Boolean(packageName && this.publicPackageProviderDeclares(occurrence, packageName));
                return {
                    occurrence,
                    permitted: !occurrence.deep &&
                        (platform ||
                            declaredPackage ||
            ...
            this.addInverse(`observed.dependency.${safeId(edge.id)}`, permitted ? 'pass' : 'fail', representative.deep
                ...
                : permitted
                    ...
                    : packageName
                        ...
                        : `Dependency does not resolve to a declared module, platform, or package: ${edge.targetModule}`, representative.location, code);
        }
        ...
        for (const edge of this.observed.inboundDependencies) {
            ...
            if (!deep || declaredSources.has(edge.sourceModule))
                continue;
            this.addInverse(`observed.dependency.inbound.${safeId(edge.id)}`, 'fail', `An unowned consumer bypasses the module entrypoint: ${deep.specifier}`, deep.location, 'MODULE_DEEP_IMPORT');
        }
    }
    publicPackageProviderDeclares(occurrence, packageName) {
        const path = occurrence.publicPath;
        if (!path?.length || occurrence.specifier !== '<public-type-closure>')
            return false;
        for (let index = path.length - 2; index >= 0; index--) {
            const owner = this.context.observedDeclarationOwners.get(path[index]);
            if (!owner)
                continue;
            return Boolean(this.context.observedModules.get(owner)?.declaredPackages.includes(packageName));
        }
        return false;
    }
