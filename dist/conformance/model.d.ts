import type { AnalysisQuery, AnalysisSnapshotSet, Completeness, FactId, ProjectUniverseId, SourceSpan } from '../analysis/index.ts';
import type { SpecificationSnapshot, SpecificationSnapshotId } from '../specification/index.ts';
...
export interface ConformanceCapabilityRequirement {
    ...
    /** Omit to require the capability in every target universe. */
    readonly universes?: readonly ProjectUniverseId[];
    /** Narrow completeness to facts owned by the current specification module. */
    readonly scope?: 'universe' | 'specification-module';
    /** Complete by default; partial evidence must be selected deliberately. */
    readonly minimumCompleteness?: 'complete' | 'partial';
    /** When partial is accepted, every reported reason must be in this closed set. */
