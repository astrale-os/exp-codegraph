export function planConformance(profiles, requestedProfiles) {
    const byId = new Map();
    for (const profile of profiles) {
        ...
        for (const requirement of manifest.requiresCapabilities) {
            ...
            if (!requirement.capability ||
                ...
                throw new Error(`Conformance profile ${manifest.id} declares an invalid capability requirement.`);
            }
        }
    ...
    for (const id of requested) {
        if (!byId.has(id))
            throw new Error(`Requested conformance profile ${id} is not installed.`);
    }
    ...
    const visit = (id) => {
        if (included.has(id))
            return;
        if (visiting.has(id))
            throw new Error(`Conformance profile dependency cycle includes ${id}.`);
        const profile = byId.get(id);
        if (!profile)
            throw new Error(`Conformance profile dependency ${id} is not installed.`);
        visiting.add(id);
        for (const dependency of [...profile.manifest.dependsOn].sort(compare))
            visit(dependency);
        visiting.delete(id);
        included.add(id);
