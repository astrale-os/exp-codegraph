...
export function compilePackageDefinition(source, text) {
    ...
    if (name && !isPackageName(name)) {
        diagnostics.push(nodeDiagnostic('PACKAGE_NAME_INVALID', `Invalid package name: ${name}`, source, parsed.file, property(object, 'package') ?? object));
    }
    return {
...
export function compilePackagePatterns(source, text) {
    ...
    for (const element of assignment.expression.elements) {
        ...
        if (!object) {
            diagnostics.push(nodeDiagnostic('PACKAGE_PATTERN_INVALID', 'Every package exception must be a literal definePackagePattern call.', source, parsed.file, element));
            continue;
        }
        ...
        if (pattern && !isPackagePattern(pattern)) {
            diagnostics.push(nodeDiagnostic('PACKAGE_PATTERN_UNSAFE', 'Package patterns must contain one terminal wildcard after a non-global package prefix.', source, parsed.file, property(object, 'pattern') ?? object));
        }
        if (pattern && reason)
            definitions.push({ pattern, reason });
    }
    const seen = new Set();
...
export function packageNameFromPath(relativePath) {
    if (!relativePath.startsWith('packages/') || !relativePath.endsWith('.ts'))
        return;
    const name = relativePath.slice('packages/'.length, -'.ts'.length);
    return isPackageName(name) ? name : undefined;
}
export function matchesPackagePattern(pattern, packageName) {
    ...
    for (const statement of file.statements) {
        if (ts.isImportDeclaration(statement)) {
            if (!ts.isStringLiteral(statement.moduleSpecifier) ||
                ...
                diagnostics.push(nodeDiagnostic('PACKAGE_IMPORT_INVALID', `Package files may import only ${AUTHORING_SPECIFIER}.`, source, file, statement));
                continue;
            }
            continue;
        }
        if (!ts.isExportAssignment(statement) || statement.isExportEquals) {
            diagnostics.push(nodeDiagnostic('PACKAGE_STATEMENT_INVALID', 'Package files may contain only an authoring import and one default export.', source, file, statement));
        }
    }
    return { file, helper, diagnostics };
}
function oneExportAssignment(file, source, diagnostics) {
    const assignments = file.statements.filter(ts.isExportAssignment);
    if (assignments.length > 1) {
        diagnostics.push(nodeDiagnostic('PACKAGE_EXPORT_AMBIGUOUS', 'Package definition files must contain exactly one default export.', source, file, assignments[1]));
    }
...
function requiredString(object, name, source, file, diagnostics) {
    ...
    if (!member || !value || value.trim() !== value) {
        diagnostics.push(nodeDiagnostic('PACKAGE_FIELD_INVALID', `Package field ${name} must be a non-empty, trimmed string literal.`, source, file, member ?? object));
        return;
    }
    return value;
}
function unknownProperties(object, allowed, source, file, diagnostics) {
    for (const member of object.properties) {
        const name = ts.isPropertyAssignment(member) ? propertyName(member.name) : undefined;
        if (!name || !allowed.has(name)) {
            diagnostics.push(nodeDiagnostic('PACKAGE_FIELD_UNKNOWN', 'Package definitions may contain only their documented literal fields.', source, file, member));
        }
    }
}
function isPackagePattern(pattern) {
    if (pattern === '*' || !pattern.endsWith('*') || pattern.slice(0, -1).includes('*'))
        return false;
