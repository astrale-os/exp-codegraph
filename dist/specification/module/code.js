...
export function compileCode(source, text) {
    ...
    for (const statement of file.statements) {
        ...
        diagnostics.push(nodeDiagnostic('CODE_STATEMENT_INVALID', 'code.ts may contain only a defineCode import and one default export.', source, file, statement));
    }
    ...
    if (!object) {
        if (assignment) {
            diagnostics.push(nodeDiagnostic('CODE_DEFINITION_INVALID', 'The default export must call defineCode with one object literal.', source, file, assignment));
        }
        return { diagnostics };
    }
    for (const member of object.properties) {
        ...
        if (name !== 'internals') {
            diagnostics.push(nodeDiagnostic('CODE_FIELD_UNKNOWN', 'Code declarations may contain only internals.', source, file, member));
        }
    }
    const member = literalProperty(object, 'internals');
    if (!member || !ts.isArrayLiteralExpression(member.initializer)) {
        diagnostics.push(nodeDiagnostic('CODE_INTERNALS_INVALID', 'internals must be a non-empty array of relative POSIX file paths.', source, file, member ?? object));
        return { diagnostics };
    }
    const internals = [];
    for (const element of member.initializer.elements) {
        ...
        if (!value || !validCodePath(value)) {
            diagnostics.push(nodeDiagnostic('CODE_INTERNAL_PATH_INVALID', 'Internal code paths must be canonical relative POSIX paths without glob syntax.', source, file, element));
            continue;
        }
        if (internals.includes(value)) {
            diagnostics.push(nodeDiagnostic('CODE_INTERNAL_PATH_DUPLICATE', `Internal code path ${value} is declared more than once.`, source, file, element));
            continue;
        }
        internals.push(value);
    }
    if (member.initializer.elements.length === 0) {
        diagnostics.push(nodeDiagnostic('CODE_INTERNALS_EMPTY', 'Remove code.ts when no shared internal entrypoint is required.', source, file, member.initializer));
    }
    return {
...
function validateImport(statement, source, file, diagnostics) {
    const bindings = statement.importClause?.namedBindings;
    const valid = ts.isStringLiteral(statement.moduleSpecifier) &&
    ...
    if (!valid) {
        diagnostics.push(nodeDiagnostic('CODE_IMPORT_INVALID', `code.ts may import only defineCode from ${AUTHORING_SPECIFIER}.`, source, file, statement));
    }
}
function validCodePath(value) {
    if (value.trim() !== value ||
        value.startsWith('/') ||
        value.includes('\\') ||
        /[*?\[\]{}]/u.test(value))
