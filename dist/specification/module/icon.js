...
export async function loadModuleIcon(file) {
    try {
...
class IconParser {
    ...
    parse() {
        try {
            ...
            if (this.index !== this.text.length)
                this.fail('Unexpected content after the SVG root.');
            if (root.name !== 'svg')
                this.fail('A module icon must have one svg root element.', 0);
    ...
    element(depth) {
        if (depth > MAX_DEPTH)
            this.fail(`Module icons cannot exceed ${MAX_DEPTH} nested elements.`);
        ...
        if (this.peek('/') || this.peek('!') || this.peek('?')) {
            this.fail('SVG declarations, comments, and unexpected closing elements are not allowed.', start);
        }
        const name = this.name();
        if (!ELEMENTS.has(name))
            this.fail(`SVG element ${name} is not allowed.`, start);
        if (depth > 0 && name === 'svg')
            this.fail('Nested svg elements are not allowed.', start);
        ...
        while (true) {
            ...
            if (this.consume('</')) {
                ...
                if (closing !== name)
                    this.fail(`Expected closing element ${name}, found ${closing}.`);
                this.whitespace();
                this.expect('>');
                return { name, attributes, children };
            }
            if (this.index >= this.text.length)
                this.fail(`SVG element ${name} is not closed.`, start);
            if (!this.peek('<'))
                this.fail('Text content is not allowed in module icons.');
    ...
    attributeValue() {
        ...
        if (quote !== '"' && quote !== "'")
            this.fail('SVG attributes must use quoted values.');
        this.index++;
        const start = this.index;
        while (this.index < this.text.length && this.text[this.index] !== quote)
            this.index++;
        if (this.index >= this.text.length)
            this.fail('SVG attribute value is not closed.', start);
...
class IconReadError extends IconError {
    text;
    constructor(message, offset, text) {
        super(message, offset);
        this.text = text;
    }
}
function validateAttribute(name, value, offset) {
    ...
    if (/\b(?:data|https?|javascript):|\/\//iu.test(value)) {
        throw new IconError(`SVG attribute ${name} cannot reference external or executable content.`, offset);
    }
