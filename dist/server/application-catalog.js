 ...
 */
export async function projectApplicationCatalog(root, reader, options = {}) {
    ...
    const projected = new Map((await projectApplicationSpecifications(root, reader, [...refresh])).map((value) => [value.source, value]));
...
export async function projectApplicationSpecifications(root, reader, sources) {
    ...
    for (let offset = 0; offset < selected.length; offset += 6) {
        ...
        specs.push(...await Promise.all(batch.map((specification) => projectSpecification(root, reader, specification.source))));
    }
    return specs;
}
