...
import { resolveAlias } from '../typescript/surface/symbol.js';
import { indexProjectBindings, observeDependencies, observeErrorCodes, observeExpectedErrorCodes, readPackageIntent, resolvedModule, } from './application-binding-evidence.js';
...
export function compileApplicationModuleBindings(options) {
    ...
    const compilationRequests = ownershipRequests.filter((request) => selectedProjects.has(request.target.project));
...
function compileProject(root, projectPath, requests, allRequests, work) {
    const configFile = resolve(root, projectPath);
    const config = ts.readConfigFile(configFile, ts.sys.readFile);
    const configError = config.error;
    if (configError) {
        return {
            facts: requests.map((request) => failedFact(root, request, [compilerDiagnostic(root, configError)])),
            sourceFiles: 0,
        };
    }
    const parsed = ts.parseJsonConfigFileContent(config.config, ts.sys, dirname(configFile), { noEmit: true }, configFile);
    ...
    return {
        facts: requests.map((request) => bindModule(root, program, project, request, index, work)),
        sourceFiles: program.getSourceFiles().length,
    };
}
function bindModule(root, program, project, request, index, work) {
    ...
    if (existsSync(binding))
        diagnostics.push(...diagnosticsForFile(root, program, binding));
    ...
    diagnostics.push(...contractSurface.issues.map((issue) => issueDiagnostic(request.source, issue.code, issue.message)), ...implementationSurface.issues.map((issue) => issueDiagnostic(request.target.entrypoint, issue.code, issue.message)));
    phase = performance.now();
    ...
    let exports = paths.map((path) => {
        ...
        if (!contract) {
            diagnostics.push(bindingDiagnostic(root, request.target.entrypoint, 'MODULE_EXPORT_UNDECLARED', `Public export is not declared by the module specification: ${path}`, path));
            return exportResult(observed, emptyFacet(), facetOf(project.checker, program, implementation, observed), 'undeclared');
        }
        if (!observed) {
            diagnostics.push(bindingDiagnostic(root, request.source, 'MODULE_EXPORT_MISSING', `Specified export is absent: ${path}`, path));
            return exportResult(contract, facetOf(project.checker, program, source, contract), emptyFacet(), 'missing');
        }
        const contractFacet = facetOf(project.checker, program, source, contract);
        const implementationFacet = facetOf(project.checker, program, implementation, observed);
        const compatible = compareExport(root, project.checker, program, source, implementation, path, contractFacet, implementationFacet, diagnostics);
        return exportResult(contract, contractFacet, implementationFacet, compatible ? 'pass' : 'incompatible');
    });
    ...
    if (!valueCompatible && exports.some((entry) => entry.contract.value)) {
        diagnostics.push(bindingDiagnostic(root, binding, existsSync(binding) ? 'MODULE_VALUE_BINDING_INVALID' : 'MODULE_VALUE_BINDING_MISSING', existsSync(binding)
            ? 'implementation.contract.ts must bind the implementation namespace to the authoritative .spec namespace with satisfies.'
            : 'Runtime exports require an implementation.contract.ts binding to the authoritative .spec namespace.', '<module>'));
        exports = exports.map((entry) => entry.contract.value && entry.status === 'pass'
    ...
    const dependencies = observeDependencies(root, program, request, index, implementationSurface.exports);
...
function compareExport(root, checker, program, contractFile, implementationFile, path, contract, implementation, diagnostics) {
    let compatible = true;
    const mismatch = (code, message, expected, actual) => {
        compatible = false;
    ...
    if (contract.typeOnly !== implementation.typeOnly) {
        mismatch('MODULE_EXPORT_TYPE_MODE_MISMATCH', `Export type/value mode differs for ${path}.`, contract.typeOnly ? 'type-only export' : 'runtime export', implementation.typeOnly ? 'type-only export' : 'runtime export');
    }
    ...
    if (contract.type && implementation.type) {
        ...
        if (!expected || !actual || !explicitTypeBinding(checker, actual, expected)) {
            mismatch('MODULE_EXPORT_TYPE_BINDING_MISSING', `Type export must resolve explicitly to the authoritative specification identity: ${path}.`);
        }
    }
    return compatible;
}
function explicitTypeBinding(checker, actual, expected) {
    if (resolveAlias(checker, actual) === expected)
        return true;
    return Boolean(actual.declarations?.some((declaration) => {
        if (!ts.isTypeAliasDeclaration(declaration))
            return false;
        const target = checker.getSymbolAtLocation(ts.isTypeReferenceNode(declaration.type)
            ? declaration.type.typeName
            : declaration.type);
        return target !== undefined && resolveAlias(checker, target) === expected;
    }));
}
function explicitValueBinding(root, program, request) {
    ...
    for (const statement of source.statements) {
        ...
        if (resolve(resolved) === resolve(root, request.target.entrypoint) &&
...
function bindingFile(root, request) {
    return resolve(root, request.target.root, 'implementation.contract.ts');
}
function exportSymbol(checker, program, file, path) {
...
function rawExportSymbol(checker, program, file, path) {
    const source = program.getSourceFile(resolve(file)) ?? program.getSourceFile(file);
    let symbol = source && checker.getSymbolAtLocation(source);
...
function facetOf(checker, program, file, value) {
...
function exportResult(value, contract, implementation, status) {
    return { path: value.path, name: value.name, contract, implementation, status };
}
function diagnosticsForFile(root, program, file) {
    const source = program.getSourceFile(resolve(file)) ?? program.getSourceFile(file);
    if (!source) {
...
function compilerDiagnostic(root, diagnostic) {
...
function issueDiagnostic(file, code, message) {
    return { code, message, file, line: 1, column: 1 };
}
function bindingDiagnostic(root, file, code, message, exportPath, expected, actual) {
    return {
        code,
        message,
...
function failedFact(_root, request, diagnostics) {
    return {
        specification: request.specification,
        target: request.target,
        exports: [],
...
function deduplicateDiagnostics(values) {
    return [...new Map(values.map((value) => [JSON.stringify(value), value])).values()].sort((left, right) => compare(`${left.file}\0${left.line}\0${left.column}\0${left.code}\0${left.exportPath ?? ''}`, `${right.file}\0${right.line}\0${right.column}\0${right.code}\0${right.exportPath ?? ''}`));
}
