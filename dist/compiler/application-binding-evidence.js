...
export function observeDependencies(root, program, request, index, exports) {
    const dependencies = [];
    for (const source of index.sourcesByOwner.get(request.target.id) ?? []) {
        const record = (specifier, node, kind, typeOnly) => {
            ...
            if (target?.id === request.target.id)
                return;
        ...
        const visit = (node) => {
            if (ts.isImportDeclaration(node) && ts.isStringLiteralLike(node.moduleSpecifier)) {
                ...
                if (!clause)
                    record(node.moduleSpecifier.text, node.moduleSpecifier, 'side-effect', false);
                else if (clause.isTypeOnly)
                    record(node.moduleSpecifier.text, node.moduleSpecifier, 'type', true);
                else {
                    const typeOnly = clause.namedBindings && ts.isNamedImports(clause.namedBindings)
                        ? clause.namedBindings.elements.some((element) => element.isTypeOnly)
                        : false;
                    const runtime = Boolean(clause.name ||
                        (clause.namedBindings &&
                            (ts.isNamespaceImport(clause.namedBindings) ||
                                clause.namedBindings.elements.some((element) => !element.isTypeOnly))));
                    if (runtime)
                        record(node.moduleSpecifier.text, node.moduleSpecifier, 'runtime', false);
                    if (typeOnly)
                        record(node.moduleSpecifier.text, node.moduleSpecifier, 'type', true);
                }
            }
    ...
    return [...new Map(dependencies.map((value) => [JSON.stringify(value), value])).values()].sort((left, right) => compare(JSON.stringify(left), JSON.stringify(right)));
}
export function readPackageIntent(moduleRoot) {
    ...
    while (true) {
        ...
        if (existsSync(file)) {
            try {
                ...
                const names = (field) => Object.keys(value[field] && typeof value[field] === 'object' && !Array.isArray(value[field])
                    ? value[field]
                    : {});
                return {
                    declared: [...new Set([
...
export function observeExpectedErrorCodes(program, file) {
    ...
    const visit = (node) => {
        for (const tag of ts.getJSDocTags(node)) {
            ...
            for (const code of comment.split(/[\s,]+/u))
...
export function observeErrorCodes(request, index) {
    const codes = new Set();
    const record = (value) => { if (ERROR_CODE.test(value))
        codes.add(value); };
    for (const source of index.sourcesByOwner.get(request.target.id) ?? []) {
        const visit = (node) => {
...
export function indexProjectBindings(root, program, requests) {
    const boundaries = requests
        .map(({ target }) => ({
        ...target,
        absoluteRoot: resolve(root, target.root),
        entrypoints: new Set([target.entrypoint, ...target.facades, ...target.aliases, ...target.internals]
            .map((path) => resolve(root, path))),
    }))
        .sort((left, right) => right.absoluteRoot.length - left.absoluteRoot.length || compare(left.id, right.id));
...
export function resolvedModule(program, specifier, source) {
    return program.getResolvedModuleFromModuleSpecifier(specifier, source)?.resolvedModule;
}
function ownerFromIndex(boundaries, file) {
    return boundaries.find((boundary) => within(boundary.absoluteRoot, file));
}
function dependencyTarget(root, specifier, resolved) {
    ...
    if (resolved) {
        ...
        if (index >= 0)
            return `package:${packageName(resolved.slice(index + marker.length).split(sep).join('/'))}`;
        return `unowned:${portable(relative(root, resolved))}`;
    }
    return `unowned:${specifier}`;
}
function packageTarget(coordinate) {
...
//# sourceMappingURL=application-binding-evidence.js.map