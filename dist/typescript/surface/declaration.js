...
import { canonicalTypeProviderCoordinate, workspacePackageCoordinate, } from '../package-coordinate.js';
import { canonicalSymbolIdentity, declarationKind, factoryFacetDeclarations, firstDeclaration, isStableDeclarationSymbol, locationOf, referencedSymbol, resolveAlias, symbolWithinCatalog, } from './symbol.js';
export function observeDeclaration(catalogRoot, checker, symbol, exportPaths, semantics) {
    ...
    const normalizer = new TypeNormalizer(catalogRoot, checker, references, issues, parameterNodes, declaration ? typeParameterScope(catalogRoot, declaration) : identity, undefined, semantics);
    ...
    if (kind === 'factory') {
        ...
        if (!facets)
            throw new Error('Factory declaration classification lost its facets.');
        const typeIssues = [];
        const typeReferences = new Map();
        const typeNormalizer = new TypeNormalizer(catalogRoot, checker, typeReferences, typeIssues, [...(facets.type.typeParameters ?? [])], typeParameterScope(catalogRoot, facets.type), undefined, semantics);
        const type = checker.getTypeAtLocation(facets.type.type);
        const typeFacetValue = typeNormalizer.normalize(type, facets.type.type);
        const authoredTypeIssues = [];
        const authoredTypeReferences = new Map();
        const authoredTypeNormalizer = new TypeNormalizer(catalogRoot, checker, authoredTypeReferences, authoredTypeIssues, [...(facets.type.typeParameters ?? [])], typeParameterScope(catalogRoot, facets.type), symbol, semantics);
        ...
        const valueIssues = [];
        const valueNormalizer = new TypeNormalizer(catalogRoot, checker, references, valueIssues, [], 'unbound', undefined, semantics);
        const valueType = checker.getTypeOfSymbolAtLocation(symbol, facets.value);
        const signatures = callablesOfType(catalogRoot, checker, valueType, facets.value, valueNormalizer, valueIssues);
        ...
        const signatures = declaration && hasOwnCallSignature(declaration)
            ? callablesOfType(catalogRoot, checker, instanceType, declaration, normalizer, issues)
            : undefined;
        const callable = signatures?.[0];
        const ownMembers = membersOfType(catalogRoot, checker, instanceType, declaration, normalizer, issues);
...
class TypeNormalizer {
    ...
    declaringAlias;
    semantics;
    constructor(catalogRoot, checker, references, issues, typeParameters = [], typeParameterScope = 'unbound', declaringAlias, semantics = 'specification-v2') {
        ...
        for (const [index, parameter] of typeParameters.entries()) {
            ...
            if (symbol)
                this.#typeParameterBindings.set(symbol, { scope: typeParameterScope, index });
        }
    }
    observeTypeParameters(parameters) {
        return parameters.map((parameter, index) => ({
            scope: this.#typeParameterBindings.get(this.checker.getSymbolAtLocation(parameter.name))?.scope ??
                'unbound',
            index,
            ...
            ...(parameter.constraint
                ? {
                    constraint: this.normalize(this.checker.getTypeFromTypeNode(parameter.constraint), parameter.constraint),
                }
                : {}),
            ...(parameter.default
                ? {
                    default: this.normalize(this.checker.getTypeFromTypeNode(parameter.default), parameter.default),
                }
                : {}),
            location: locationOf(this.catalogRoot, parameter),
        }));
    }
    bindTypeParameters(parameters, scope) {
        const previous = [];
        ...
        return () => {
            for (const [symbol, index] of previous) {
                if (index === undefined)
                    this.#typeParameterBindings.delete(symbol);
                else
                    this.#typeParameterBindings.set(symbol, index);
            }
        };
    }
    normalize(type, location) {
        const authoredAlgebra = this.semantics === 'specification-v2'
        ...
        if (authoredAlgebra) {
            return {
                ...
                types: canonicalTypes(authoredAlgebra.types.map((item) => this.normalize(this.checker.getTypeFromTypeNode(item), item))),
            };
        }
        if (this.#active.has(type)) {
            ...
            if (opaqueUnion) {
                return {
                    kind: 'union',
                    types: canonicalTypes(opaqueUnion.map((item) => this.normalize(this.checker.getTypeFromTypeNode(item), item))),
                };
            }
    ...
    normalizeActive(type, location) {
        ...
        const locationAlias = ts.isTypeAliasDeclaration(location)
            ...
            : undefined;
        const declaringAlias = Boolean(alias && this.declaringAlias && resolveAlias(this.checker, this.declaringAlias) === alias) ||
        ...
        if (sourceType && ts.isConditionalTypeNode(sourceType)) {
            return {
                kind: 'conditional',
                check: this.normalize(this.checker.getTypeFromTypeNode(sourceType.checkType), sourceType.checkType),
                extends: this.normalize(this.checker.getTypeFromTypeNode(sourceType.extendsType), sourceType.extendsType),
                trueType: this.normalize(this.checker.getTypeFromTypeNode(sourceType.trueType), sourceType.trueType),
                falseType: this.normalize(this.checker.getTypeFromTypeNode(sourceType.falseType), sourceType.falseType),
            };
        }
        if (sourceType &&
        ...
        if (sourceType && ts.isIndexedAccessTypeNode(sourceType)) {
            return {
                kind: 'indexed-access',
                object: this.normalize(this.checker.getTypeFromTypeNode(sourceType.objectType), sourceType.objectType),
                index: this.normalize(this.checker.getTypeFromTypeNode(sourceType.indexType), sourceType.indexType),
            };
        }
        const opaqueUnion = authoredOpaqueUnknownUnion(this.checker, type, location);
        if (opaqueUnion) {
        ...
        if (type.flags & ts.TypeFlags.Any)
            return this.unsupported('any', type, location);
        ...
        if (type.flags & ts.TypeFlags.TypeParameter) {
            ...
            if (binding)
                return { kind: 'parameter', ...binding };
            return this.unsupported('unbound type parameter', type, location);
        }
        if (type.flags & ts.TypeFlags.TemplateLiteral) {
            const template = type;
            const sourceType = unwrappedTypeNodeAt(location);
            const nodes = sourceType && ts.isTemplateLiteralTypeNode(sourceType)
                ? sourceType.templateSpans.map((span) => span.type)
                : undefined;
            return {
                kind: 'template',
                texts: [...template.texts],
                types: template.types.map((item, index) => this.normalize(item, nodes?.[index] ?? location)),
            };
        }
        ...
        if (type.isUnion()) {
            ...
            if (sourceType &&
                ...
                return {
                    kind: 'union',
                    types: canonicalTypes(sourceType.types.map((item) => this.normalize(this.checker.getTypeFromTypeNode(item), item))),
                };
            }
            const nodes = sourceType && ts.isUnionTypeNode(sourceType)
                ? constituentTypeNodes(this.checker, type.types, sourceType.types)
                : undefined;
            return {
                kind: 'union',
                types: canonicalTypes(type.types.map((item, index) => this.normalize(item, nodes?.[index] ?? location))),
            };
        }
        if (type.isIntersection()) {
            ...
            const nodes = sourceType && ts.isIntersectionTypeNode(sourceType)
                ? constituentTypeNodes(this.checker, type.types, sourceType.types)
                : undefined;
            return {
                kind: 'intersection',
        ...
        if (this.checker.isTupleType(type)) {
            ...
            const sourceType = unwrappedTypeNodeAt(location);
            const elementNodes = sourceType && ts.isTupleTypeNode(sourceType) ? sourceType.elements : undefined;
            return {
                kind: 'tuple',
                elements: this.checker
                    .getTypeArguments(reference)
                    .map((item, index) => this.normalize(item, tupleElementLocation(this.checker, item, elementNodes?.[index]) ?? location)),
        ...
        if (type.flags & ts.TypeFlags.Object) {
            const callSignatureCount = this.checker.getSignaturesOfType(type, ts.SignatureKind.Call).length;
            if (callSignatureCount > 0) {
                const signatures = callablesOfType(this.catalogRoot, this.checker, type, location, this, this.issues);
                const callable = signatures?.[0];
                if (callable) {
                    return {
                        kind: 'function',
            ...
            if (this.semantics === 'specification-v2' && constructSignatures.length > 0) {
                const signatures = callablesOfType(this.catalogRoot, this.checker, type, location, this, this.issues, ts.SignatureKind.Construct);
                const callable = signatures?.[0];
                if (callable) {
                    return {
                        kind: 'constructor',
            ...
            const members = membersOfType(this.catalogRoot, this.checker, type, undefined, this, this.issues);
            return { kind: 'object', members };
        }
        return this.unsupported('type', type, location);
    }
    normalizeAuthoredType(type, node) {
        if (!node)
            return;
        ...
        if (this.checker.getTypeFromTypeNode(node) !== type)
            return;
        if (ts.isConditionalTypeNode(node)) {
            return {
                kind: 'conditional',
                check: this.normalize(this.checker.getTypeFromTypeNode(node.checkType), node.checkType),
                extends: this.normalize(this.checker.getTypeFromTypeNode(node.extendsType), node.extendsType),
                trueType: this.normalize(this.checker.getTypeFromTypeNode(node.trueType), node.trueType),
                falseType: this.normalize(this.checker.getTypeFromTypeNode(node.falseType), node.falseType),
            };
        }
        ...
        if (ts.isIndexedAccessTypeNode(node)) {
            return {
                kind: 'indexed-access',
                object: this.normalize(this.checker.getTypeFromTypeNode(node.objectType), node.objectType),
        ...
        if (ts.isTemplateLiteralTypeNode(node)) {
            return {
                ...
                types: node.templateSpans.map((span) => this.normalize(this.checker.getTypeFromTypeNode(span.type), span.type)),
            };
        }
        if (ts.isFunctionTypeNode(node) || ts.isConstructorTypeNode(node)) {
            ...
            const signatures = callablesOfType(this.catalogRoot, this.checker, type, node, this, this.issues, signatureKind);
            const callable = signatures?.[0];
            if (!callable)
                return this.unsupported('callable type', type, node);
            return {
        ...
        if (ts.isLiteralTypeNode(node))
            return literalType(node, type);
        return;
    }
    unsupported(reason, type, node) {
        let display;
        try {
    ...
    referenceArguments(type, location) {
        ...
        if (node && (ts.isTypeReferenceNode(node) || ts.isImportTypeNode(node))) {
            return (node.typeArguments ?? []).map((argument) => this.normalize(this.checker.getTypeFromTypeNode(argument), argument));
        }
...
function callablesOfSymbol(catalogRoot, checker, symbol, normalizer, issues) {
    const declaration = firstDeclaration(symbol);
    if (!declaration)
        return undefined;
...
function callablesOfType(catalogRoot, checker, type, declaration, normalizer, issues, signatureKind = ts.SignatureKind.Call) {
    const signatures = checker.getSignaturesOfType(type, signatureKind);
    if (!signatures.length)
        return undefined;
    return signatures.map((signature) => callableOfSignature(catalogRoot, checker, signature, declaration, normalizer, issues));
}
function callableOfSignature(catalogRoot, checker, signature, declaration, normalizer, issues) {
    const signatureDeclaration = signature.getDeclaration();
    const typeParameterNodes = signatureDeclaration?.typeParameters
        ? [...signatureDeclaration.typeParameters]
        : [];
    const restoreTypeParameters = normalizer.bindTypeParameters(typeParameterNodes, typeParameterScope(catalogRoot, signatureDeclaration ?? declaration));
    ...
    if (signature.thisParameter) {
        issues.push({
            ...
            location: locationOf(catalogRoot, signature.thisParameter.valueDeclaration ??
    ...
    try {
        const parameters = signature.getParameters().map((parameter, index) => {
            const parameterDeclaration = parameter.valueDeclaration ?? firstDeclaration(parameter) ?? declaration;
            const type = checker.getTypeOfSymbolAtLocation(parameter, parameterDeclaration);
            const rest = ts.isParameter(parameterDeclaration) && Boolean(parameterDeclaration.dotDotDotToken);
            const optional = Boolean(parameter.flags & ts.SymbolFlags.Optional) ||
                (ts.isParameter(parameterDeclaration) &&
                    Boolean(parameterDeclaration.questionToken || parameterDeclaration.initializer));
            const authoredOptionalType = normalizer.semantics === 'specification-v2' &&
                optional &&
                ts.isParameter(parameterDeclaration) &&
        ...
        const assertedReturnNode = normalizer.semantics === 'specification-v2' && !returnNode
            ? callableReturnAssertionTypeNode(signatureDeclaration ?? declaration)
            : undefined;
        const returnEvidenceNode = returnNode ?? assertedReturnNode;
        const normalizedReturnNode = promised && returnEvidenceNode && ts.isTypeReferenceNode(returnEvidenceNode)
...
function bigintLiteral(type, location) {
...
function literalType(node, type) {
...
function containingTypeSymbol(checker, node) {
...
function membersOfType(catalogRoot, checker, type, ownerDeclaration, normalizer, issues) {
    const members = [];
    for (const symbol of checker.getPropertiesOfType(type)) {
        ...
        const signatures = isMethodDeclaration(declaration)
            ? callablesOfType(catalogRoot, checker, optional ? checker.getNonNullableType(memberType) : memberType, declaration, normalizer, issues)
            : undefined;
        const callable = signatures?.[0];
...
function staticMembersOfClass(catalogRoot, checker, symbol, declaration, normalizer, issues) {
    if (!declaration)
        return undefined;
    const computedStatics = new Set();
    ...
    const members = checker.getPropertiesOfType(constructorType).flatMap((member) => {
        const memberDeclaration = (member.declarations ?? []).find((candidate) => candidate.parent === declaration && hasStaticModifier(candidate));
        if (!memberDeclaration || member.getName() === 'prototype')
            return [];
        const memberName = memberDeclaration.name;
        ...
        const signatures = callablesOfType(catalogRoot, checker, optional ? checker.getNonNullableType(type) : type, memberDeclaration, normalizer, issues);
        const callable = signatures?.[0];
        return [
            {
                name: member.getName(),
...
function publicMemberIdentity(catalogRoot, checker, symbol, declaration, issues) {
...
function heritageOf(catalogRoot, checker, declaration, references, issues) {
    if (!declaration ||
    ...
    for (const clause of declaration.heritageClauses ?? []) {
        for (const typeNode of clause.types) {
            ...
            const symbol = referencedSymbol(type) ?? (syntaxSymbol ? resolveAlias(checker, syntaxSymbol) : undefined);
...
function recordTypeNodes(location) {
    const node = typeNodeAt(location);
    ...
    if (!ts.isTypeReferenceNode(inner) || inner.typeName.getText() !== 'Record')
        return;
    if (inner.typeArguments?.length !== 2)
        return;
...
function builtInReferenceType(checker, type, location) {
    const node = typeNodeAt(location);
    ...
    if (!BUILT_IN_REFERENCE_TYPES.has(name))
        return;
    if (target.declarations?.some((declaration) => declaration.getSourceFile().hasNoDefaultLib ||
...
function declaredValueType(checker, symbol, declaration) {
    if (!declaration)
        return undefined;
...
function declarationTypeParameters(declaration) {
    return declaration &&
...
function firstPublicMemberDeclaration(symbol, owner) {
    return (symbol.declarations ?? []).find((declaration) => {
        if (owner && !belongsTo(declaration, owner))
            return false;
        const name = declaration.name;
        if (name && ts.isPrivateIdentifier(name))
            return false;
        return (!hasModifier(declaration, ts.SyntaxKind.PrivateKeyword) &&
            !hasModifier(declaration, ts.SyntaxKind.ProtectedKeyword));
    });
}
function belongsTo(declaration, owner) {
...
function reportDeclarationMerging(symbol, kind, catalogRoot, issues) {
    if (kind === 'factory')
        return;
    const declarations = symbol.declarations ?? [];
    const kinds = new Set(declarations.map((declaration) => declaration.kind));
    const overloadOnly = kind === 'callable' &&
        [...kinds].every((value) => value === ts.SyntaxKind.FunctionDeclaration || value === ts.SyntaxKind.MethodSignature);
    const classNamespace = kind === 'class' &&
        declarations.filter(ts.isClassDeclaration).length === 1 &&
        declarations.every((declaration) => ts.isClassDeclaration(declaration) || ts.isModuleDeclaration(declaration));
...
function reportUnsupportedDeclaredShape(catalogRoot, checker, type, declaration, issues) {
    if (!declaration)
        return;
    const ownIndexSignatures = ts.isInterfaceDeclaration(declaration) || ts.isClassDeclaration(declaration)
...
function explicitReferenceSymbol(checker, location) {
...
function unwrappedTypeNodeAt(node) {
...
function tupleElementLocation(checker, type, node) {
    if (!node)
        return undefined;
    ...
    if (rest) {
        ...
        if (unwrapped && ts.isArrayTypeNode(unwrapped))
    ...
    if (ts.isNamedTupleMember(node) || ts.isOptionalTypeNode(node))
        return node.type;
    return node;
}
function typeNodeAt(node) {
    if (ts.isTypeNode(node))
        return node;
    if ('type' in node) {
        const type = node.type;
        if (type && typeof type === 'object' && 'kind' in type && ts.isTypeNode(type)) {
            return type;
        }
    }
...
function assertionTypeNode(expression) {
...
function callableReturnTypeNode(declaration) {
 ...
 */
function callableReturnAssertionTypeNode(declaration) {
    let expression;
...
function callableInferenceLocation(declaration) {
    if (ts.isArrowFunction(declaration) && !ts.isBlock(declaration.body))
...
function typeNodeContainsTypeParameter(checker, node) {
    let found = false;
    const visit = (current) => {
        if (found)
            return;
...
function isReadonlyTuple(type) {
    return Boolean(type.target?.readonly);
}
 ...
 */
function constituentTypeNodes(checker, types, nodes) {
    const candidates = nodes.map((node) => ({ node, type: checker.getTypeFromTypeNode(node) }));
    const claimed = new Set();
    return types.map((type) => {
        const index = candidates.findIndex((candidate, candidateIndex) => !claimed.has(candidateIndex) && candidate.type === type);
        if (index < 0)
            return undefined;
        claimed.add(index);
...
function hasExplicitExternalConstituent(catalogRoot, checker, nodes) {
    return nodes.some((node) => {
        const symbol = explicitReferenceSymbol(checker, node);
        return symbol !== undefined && externalTypeSymbol(catalogRoot, symbol);
    });
}
function externalTypeSymbol(catalogRoot, symbol) {
    return (!symbolWithinCatalog(catalogRoot, symbol) ||
        Boolean(symbol.declarations?.some((declaration) => basename(dirname(declaration.getSourceFile().fileName)) === '.astrale-spec-externals')));
}
/** Preserve an authored external intersection even when opaque stubs collapse its checker type. */
function authoredExternalIntersection(catalogRoot, checker, location) {
...
function externalAuthoredIntersection(catalogRoot, checker, location) {
...
function authoredAlgebraicType(checker, type, location) {
    const source = unwrappedTypeNodeAt(location);
    if (!source || checker.getTypeFromTypeNode(source) !== type)
        return;
    if (ts.isUnionTypeNode(source))
        return { kind: 'union', types: source.types };
    if (ts.isIntersectionTypeNode(source))
        return { kind: 'intersection', types: source.types };
    return;
}
/** Recover authored unions collapsed to TypeScript's shared `unknown` sentinel by opaque imports. */
function authoredOpaqueUnknownUnion(checker, type, location) {
    if (!(type.flags & ts.TypeFlags.Unknown))
        return;
    const source = unwrappedTypeNodeAt(location);
    if (!source || !ts.isUnionTypeNode(source))
        return;
    if (source.types.some((item) => item.kind === ts.SyntaxKind.UnknownKeyword))
        return;
