...
export function observePublicSurface(catalogRoot, project, entrypoint, options = {}) {
    ...
    if (source.isDeclarationFile && !options.explicitExportsOnly) {
        issues.push({
            ...
            message: 'Declaration-only implementation entrypoints require a declaration-package observer.',
            location: locationOf(catalogRoot, source),
        });
    }
    collectExports(catalogRoot, project.checker, rootSymbol, [], exports, symbols, paths, issues, new Set(), options.explicitExportsOnly ?? false);
    ...
    for (let index = 0; index < pending.length; index++) {
        ...
        if (declarations.has(identity))
            continue;
        if (!symbolWithinCatalog(catalogRoot, symbol) ||
    ...
    return {
        exports,
        declarations: [...declarations.values()].sort((left, right) => compare(left.identity, right.identity)),
        issues: deduplicateIssues(issues),
    };
}
function symbolOwnedByFiles(symbol, files) {
    return Boolean(symbol.declarations?.some((declaration) => files.has(declaration.getSourceFile().fileName)));
}
function collectExports(catalogRoot, checker, moduleSymbol, namespace, output, symbols, paths, issues, active, explicitExportsOnly) {
    ...
    const exportedSymbols = checker
        ...
        .sort((left, right) => compareExportDeclarations(left.exported, left.target, right.exported, right.target) ||
        left.index - right.index);
    for (const { exported, target } of exportedSymbols) {
        if (explicitExportsOnly && !isExplicitExport(exported, target))
            continue;
        const name = exported.getName();
        if (name === 'prototype' && !firstDeclaration(exported) && !firstDeclaration(target))
            continue;
        const path = [...namespace, name];
        const kind = declarationKind(checker, target);
        if (kind === 'namespace' && isPureNamespaceSymbol(target)) {
            collectExports(catalogRoot, checker, target, path, output, symbols, paths, issues, active, explicitExportsOnly);
            continue;
        }
...
function exportSourceModule(catalogRoot, checker, exported, active = new Set()) {
    if (active.has(exported))
        return {};
    active.add(exported);
    for (const declaration of exported.declarations ?? []) {
        ...
        if (!moduleDeclaration)
            continue;
        const coordinate = workspacePackageCoordinate(catalogRoot, moduleDeclaration.getSourceFile().fileName);
    ...
    if (resolved !== exported) {
        if (symbolWithinCatalog(catalogRoot, resolved))
            return {};
        const declaration = firstDeclaration(resolved);
        const coordinate = declaration && workspacePackageCoordinate(catalogRoot, declaration.getSourceFile().fileName);
        const sourceModule = coordinate && publicPackageCoordinate(coordinate);
        if (sourceModule)
            return { sourceModule };
    }
...
function publicPackageSpecifier(specifier) {
...
function publicPackageCoordinate(coordinate) {
    if (!coordinate.startsWith('package:'))
        return;
...
function compareExportDeclarations(leftExport, leftTarget, rightExport, rightTarget) {
    ...
    if (!right)
        return -1;
    return (compare(left.getSourceFile().fileName, right.getSourceFile().fileName) || left.pos - right.pos);
}
function isExplicitExport(exported, target) {
