...
const declaredLanguages = new Map([
...
export function highlightSourceCode(code, declaredLanguage) {
    if (!code)
        return;
    const declared = normalizeLanguage(declaredLanguage);
    if (declared && plainLanguages.has(declared))
        return;
    ...
    try {
        const tokens = [];
        highlightCode(code, selected.parser.parse(code), classHighlighter, (text, classes) => tokens.push(classes ? { text, classes } : { text }), () => tokens.push({ text: '\n' }));
...
export function highlightedSourceHtml(source) {
...
function looksLikeTypeScript(code) {
    return (/(^|\n)\s*(?:export\s+)?(?:declare\s+)?(?:abstract\s+)?(?:interface|type|enum|namespace)\s+[A-Za-z_$]/m.test(code) ||
