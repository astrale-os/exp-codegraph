...
export async function locateResource(root, containingFile, reference, suffix) {
    ...
    if (!within(catalogRoot, absolute))
        throw new Error('Resource resolves outside the catalog root.');
    return { absolute, source: portable(relative(catalogRoot, absolute)) };
}
function validateReference(reference, suffix) {
    if (!reference ||
        ...
        throw new Error('Resource references must be relative POSIX paths without query or fragment components.');
    }
    if (!reference.endsWith(suffix))
        throw new Error(`Resource reference must end with ${suffix}.`);
}
