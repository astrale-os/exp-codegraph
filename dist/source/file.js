...
export async function readBytesBounded(file, maximumBytes = MAX_FILE_BYTES) {
...
export async function readBounded(file, maximumBytes = MAX_FILE_BYTES) {
...
export async function optionalDirectFile(file) {
    try {
        const handle = await openDirectFile(file);
        await handle.close();
        return true;
...
export async function replaceBounded(file, text, expectedRevision) {
...
async function openDirectFile(file) {
    const before = await lstat(file);
    if (before.isSymbolicLink())
        throw new Error('Symbolic links are not supported.');
    if (!before.isFile())
