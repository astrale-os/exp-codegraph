...
export function isWatchedSource(
  snapshot: TypeSpecApplicationSnapshot | undefined,
  root: string,
  file: string,
...
function explicitSource(specification: SpecificationSnapshot, source: string): boolean {
  const moduleRoot = specification.root
  if (
    source === (moduleRoot === '.' ? 'package.json' : `${moduleRoot}/package.json`) ||
...
function specificationSources(specification: SpecificationSnapshot): ReadonlySet<string> {
  const resources = [
...
function ignoredWorkspaceOutput(source: string): boolean {
  ...
  for (let index = 0; index < segments.length - 1; index++) {
    ...
    if (
      ((parent === 'benchmark' || parent === 'benchmarks' || parent === 'qualification') &&
