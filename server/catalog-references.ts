...
import type {
  CatalogLawReferences,
  CatalogSemanticReference,
  CatalogSemanticReferences,
  CatalogSemanticReferenceTarget,
} from '../viewer-host/catalog.ts'

import { markdownSemanticMentions } from '../specification/module/markdown-reference.ts'
...
type CandidateChoices = ReadonlyMap<string, ReadonlyMap<string, Candidate>>

const identifier = /^[$_\p{ID_Start}][$\u200c\u200d\p{ID_Continue}]*$/u
const mention =
  /[$_\p{ID_Start}][$\u200c\u200d\p{ID_Continue}]*(?:\.[$_\p{ID_Start}][$\u200c\u200d\p{ID_Continue}]*)*/gu
const callableKinds = new Set<CatalogSemanticReferenceTarget['kind']>(['callable', 'factory'])
...
export function catalogReferenceProjection(
  spec: ViewerSpecification,
  index: ApiCatalogIndex,
): CatalogReferenceProjection {
  const documents = new Map<object, readonly CatalogSemanticReference[]>()
  const candidates = referenceCandidates(spec, index)
  if (candidates.size === 0) return { documents }

  const laws: Record<string, CatalogLawReferences> = {}
...
function projectDocuments(
  ...
  output: Map<object, readonly CatalogSemanticReference[]>,
): void {
  ...
  for (const document of documents) {
    ...
    if (references.length) output.set(document, references)
  }
}

function referencesInMarkdown(
  document: MarkdownDocument,
  candidates: CandidateChoices,
): CatalogSemanticReference[] {
  const references: CatalogSemanticReference[] = []
  for (const mention of markdownSemanticMentions(document)) {
    ...
    if (references.length > MAX_SEMANTIC_REFERENCES_PER_DOCUMENT) return []
  }
  return references
}

function referenceCandidates(spec: ViewerSpecification, index: ApiCatalogIndex): CandidateChoices {
  const targets = new Map<string, Map<string, Candidate>>()
  for (const module of spec.modules) {
    const api = module.api?.model
    if (!api) continue
    const declarations = new Map(api.surface.declarations.map((item) => [item.identity, item]))
    const visibleLabels = new Map<string, Set<string>>()
    const sources = new Map(
      api.sources.flatMap((source) =>
        source.text === undefined ? [] : [[source.file, source.text] as const],
      ),
    )
    ...
    for (const token of api.tokens) {
      if (!token.target || !identifier.test(token.text)) continue
    ...
    for (const declaration of api.surface.declarations) {
      ...
      for (const member of members) {
        if (!identifier.test(member.name)) continue
...
function addVisibleCandidate(
  candidates: Map<string, Map<string, Candidate>>,
  visibleLabels: Map<string, Set<string>>,
  identity: string,
  label: string,
  target: CatalogSemanticReferenceTarget,
): void {
  addCandidate(candidates, label, target)
  const labels = visibleLabels.get(identity) ?? new Set<string>()
  labels.add(label)
...
function addCandidate(
  ...
  label: string,
  target: CatalogSemanticReferenceTarget,
  callable = callableKinds.has(target.kind),
): void {
  if (!label || !label.split('.').every((part) => identifier.test(part))) return
  const choices = candidates.get(label) ?? new Map<string, Candidate>()
...
function declarationTarget(
  index: ApiCatalogIndex,
  identity: string,
  kind: CatalogSemanticReferenceTarget['kind'],
): CatalogSemanticReferenceTarget | undefined {
...
function declarationOwner(index: ApiCatalogIndex, identity: string) {
...
function parentDeclarationIdentity(identity: string): string | undefined {
  const separator = identity.lastIndexOf('#')
  const first = identity.indexOf('#')
  return separator > first ? identity.slice(0, separator) : undefined
}

function referencesIn(value: string, candidates: CandidateChoices): CatalogSemanticReference[] {
  const references: CatalogSemanticReference[] = []
  for (const match of value.matchAll(mention)) {
    const from = match.index
    const text = match[0]
    if (from > 0 && value[from - 1] === '\\') continue
    const exact = resolveCandidate(candidates, text, false)
    if (exact) references.push(reference(from, text, exact))
  }
  return references
}

function reference(from: number, text: string, candidate: Candidate): CatalogSemanticReference {
