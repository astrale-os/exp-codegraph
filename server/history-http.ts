...
export async function handleHistoryResourceHttp(
  request: IncomingMessage,
  response: ServerResponse,
  root: string,
  lookup: HistoryResourceLookup,
): Promise<boolean> {
  if (!request.url) return false
  const url = new URL(request.url, 'http://127.0.0.1')
  if (url.pathname !== HISTORY_RESOURCE_ENDPOINT) return false
  ...
  response.setHeader(
    'content-disposition',
    `${resource.presentation === 'binary' ? 'attachment' : 'inline'}; filename*=UTF-8''${encodeURIComponent(resource.name)}`,
  )
  if (range) response.setHeader('content-range', `bytes ${start}-${end}/${verified.size}`)
...
async function openVerifiedHistory(
  target: string,
  resource: HistoryResource,
): Promise<VerifiedHistory | undefined> {
  ...
  try {
    ...
    while (offset < before.size) {
      ...
      if (!bytesRead) throw new Error('History resource ended while it was verified.')
...
function servedMediaType(resource: HistoryResource): string {
...
function byteRange(value: string | undefined, size: number): Range | 'invalid' | undefined {
