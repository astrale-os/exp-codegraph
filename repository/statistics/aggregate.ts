...
import { emptySourceLines } from './lines.ts'
...
export function summarizeRepositoryStatistics(
  ...
  const largest = [...files].sort(
    (left, right) =>
      right.lines.code - left.lines.code ||
...
export function aggregateRepositoryStatistics(
  ...
  const statisticsBySource = new Map(files.map((file) => [file.source, file] as const))
  const groups = new Map<string, { dimension: string; key: string; label: string; files: RepositoryFileStatistics[] }>()
...
export function mergeStatisticsCompleteness(values: readonly Completeness[]): Completeness {
...
export function sumSourceLines(values: readonly SourceLineMetrics[]): SourceLineMetrics {
  return values.reduce<SourceLineMetrics>(
    (total, value) => ({
...
function scalarGrouping(
  id: string,
  value: (file: RepositoryFile) => string,
): RepositoryStatisticsGrouping {
...
function uniqueValues(
  ...
  return [...new Map(values.map((value) => [value.key, value] as const)).values()].sort((left, right) =>
...
function uniqueReasons<T extends { readonly code: string; readonly message: string }>(
  reasons: readonly T[],
): readonly T[] {
  return [...new Map(reasons.map((reason) => [`${reason.code}\u0000${reason.message}`, reason] as const)).values()].sort(
