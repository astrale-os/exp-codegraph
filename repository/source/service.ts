...
export function createRepositorySourceService(
  ...
  const maximumTextBytes =
    options.maximumTextBytes ?? DEFAULT_REPOSITORY_SOURCE_MAXIMUM_TEXT_BYTES
  ...
  const bySource = new Map(inventory.files.map((file) => [file.source, file] as const))
  const byPath = new Map(inventory.files.map((file) => [file.path, file] as const))
  return {
    inventory: inventory.revision,
    async read(request: RepositorySourceRequest): Promise<RepositorySourceRead> {
