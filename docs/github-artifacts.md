...
manifest and the five platform manifests are marked `private`, omit `publishConfig`, and exist as
...
6. `@astrale-os/codegraph-native-win32-x64`
...
2. builds and qualifies each of the five native targets;
3. assembles manifests that bind every executable to that source revision;
4. packs the root and all five native units into six tarballs;
...
environment. Its lock may reference only the six downloaded artifact tarballs plus ordinary public
...
The source workspace links the five owned native units for development. It also applies the strict
