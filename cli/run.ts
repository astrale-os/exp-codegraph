...
export interface CliServices {
  ...
  executeEvidenceTests(
    ...
    onGroup?: (group: EvidenceTestPlan['groups'][number]) => void,
...
export async function runCommand(
  ...
  const changed =
    command.name === 'changed' || (command.name === 'test' && command.changed)
  ...
  try {
    ...
    if (command.name === 'test') {
      ...
      if (applicationFailed(snapshot, diagnostics)) {
        output.out(
          `Evidence tests not started: ${diagnostics.length} specification diagnostic${diagnostics.length === 1 ? '' : 's'}.`,
        )
        return { exitCode: 1 }
      }
    ...
    for (const diagnostic of snapshot.diagnostics) printDiagnostic(output, diagnostic)
    ...
    if (command.requirePass && !allPass) {
      output.error(
        `Pass-required verification expected ${expected} passing specification${expected === 1 ? '' : 's'}; found ${counts.pass}.`,
      )
    }
    return {
...
function refreshOptions(
  ...
  if (command.name === 'check' || command.name === 'changed') {
    const select =
      command.name === 'changed' && changed?.kind === 'selected'
        ? changed.targets
        : command.name === 'check'
          ? command.select
          : []
...
function reportCheck(
  ...
  if (command.name === 'changed' && changed?.kind === 'full') {
    ...
    output.out(
      `${prefix} ${command.select.length ? selected.length : checked} specification${(command.select.length ? selected.length : checked) === 1 ? '' : 's'}${supportText}: ${suffix}`,
    )
  }
}

async function executeEvidenceCommand(
  command: Extract<CliCommand, { name: 'test' }>,
  services: CliServices,
  output: CliOutput,
  plan: EvidenceTestPlan,
): Promise<CliResult> {
  ...
  output.out(
    `Testing ${plan.active} active evidence declaration${plan.active === 1 ? '' : 's'} in ${fileCount} file${fileCount === 1 ? '' : 's'} across ${plan.groups.length} package${plan.groups.length === 1 ? '' : 's'} (${plan.skipped} skipped, ${plan.todo} todo).`,
  )
  const result = await services.executeEvidenceTests(
    command.root,
    plan,
    command.quiet
      ? undefined
      : (group) =>
          output.out(
            `Testing ${group.packageName}: ${group.files.length} file${group.files.length === 1 ? '' : 's'}, ${group.evidenceCount} evidence declaration${group.evidenceCount === 1 ? '' : 's'}.`,
          ),
  )
  output.out(
    `Tested ${plan.groups.length} package${plan.groups.length === 1 ? '' : 's'}: ${result.passed} passed, ${result.failed} failed.`,
  )
  output.out(
    command.changed
      ? 'Changed-only evidence is advisory; pnpm test remains authoritative.'
...
function qualificationCounts(values: readonly QualificationSnapshot[]): {
  pass: number
  fail: number
  idle: number
  error: number
  ...
  return values.reduce(
    (counts, value) => {
      ...
      else if (value.status === 'indeterminate') counts.idle += 1
      else counts.error += 1
      return counts
    },
...
function deduplicateDiagnostics(values: readonly Diagnostic[]): readonly Diagnostic[] {
