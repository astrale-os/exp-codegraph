...
import { createCliAccelerationEvent } from '../acceleration.ts'
import {
  CHECK_CATALOG_ARTIFACT,
  CHECK_RESULT_ARTIFACT,
  CHECK_SEMANTIC_PLAN,
  MAXIMUM_CHECK_CATALOG_BYTES,
  MAXIMUM_CHECK_RESULT_BYTES,
  SEMANTIC_PACK_FORMAT,
  SEMANTIC_PACK_VERSION,
  isApplicationCheckpointReference,
  isCheckSemanticPlan,
...
export async function publishSemanticPack(
  ...
  try {
    ...
    return {
      ...createCliAccelerationEvent(
        'semantic-pack-publish',
        'published',
        options.application ? 'published-with-application' : 'published-compact',
        started,
      ),
      work: {
    ...
    return createCliAccelerationEvent(
      'semantic-pack-publish',
      'failed',
      'publication-failed',
      started,
...
export async function loadSemanticPack(
  store: FileWorkspaceCheckpointStore,
  scope: string,
  expectation: {
...
): Promise<SemanticPackLoad> {
  const started = performance.now()
  const miss = (
    code: string,
    application?: ApplicationCheckpointReference,
  ): SemanticPackLoad => ({
  ...
  try {
    ...
    if (!manifest.ok) return miss(manifest.reason)
    const payload = manifest.manifest.payload
    if (
      manifest.manifest.format !== SEMANTIC_PACK_FORMAT ||
    ...
    const identity = payload as { readonly [key: string]: unknown }
    if (
      identity.sourceProof !== expectation.sourceProof ||
      identity.repository !== expectation.repository ||
      identity.family !== expectation.family
    ) return miss('identity-mismatch')
    if (!isCheckSemanticPlan(identity.plan)) return miss('manifest-incompatible')
    ...
    if (!selected.ok) return miss(selected.reason, application)
    const bytes = selected.artifacts.get(key)
    if (!bytes) return miss('artifact-missing', application)
    if (exact) {
      const decoded = decodeWorkspaceCheckpointJson(bytes, {
        maximumDecodedBytes: MAXIMUM_CHECK_RESULT_BYTES,
      })
      const result = decoded.value
      if (!isStoredCheckResult(result)) return miss('payload-invalid', application)
      if (
        result.producerFingerprint !== expectation.producerFingerprint ||
    ...
    if (!isStoredCheckCatalog(catalog)) return miss('payload-invalid', application)
    if (
      catalog.producerFingerprint !== expectation.producerFingerprint ||
    ...
    return {
      event: createCliAccelerationEvent(
        'semantic-pack-read',
        'failed',
        'load-failed',
        started,
...
export async function portableApplicationReference(
  store: FileWorkspaceCheckpointStore,
...
): Promise<ApplicationCheckpointReference | undefined> {
  const admitted = await admitApplicationCheckpointManifest(
    ...
    {
      repository: repository as ApplicationCheckpointManifestExpectation['repository'],
      inventory: inventory as ApplicationCheckpointManifestExpectation['inventory'],
