...
export function hashGenerationIdentity(
  ...
  for (let index = 0; index < input.manifest.length; index++) {
    ...
    if (!REFERENCES.has(reference) && !flatRecord(reference)) return generic()
...
function encodeReference(reference: FactShardReference): string {
  const previous = REFERENCES.get(reference)
  if (previous !== undefined) return previous
  const encoded = stableJson(reference)
  if (Object.isFrozen(reference)) REFERENCES.set(reference, encoded)
  return encoded
}
