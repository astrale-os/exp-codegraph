...
import type { AnalysisPolicy, PolicyEvaluation, PolicyRuleResult, PolicyRuleStatus, PolicyRunOptions } from './model.ts'

/** Evaluate installed policy code over one immutable pinned query without a store or commit port. */
export async function runAnalysisPolicies(options: PolicyRunOptions): Promise<PolicyEvaluation> {
  options.signal?.throwIfAborted()
  const capabilities = new Map(
    (await options.query.capabilities()).map((status) => [status.capability, status.completeness]),
  )
  const schemas = new Map(
    (await options.query.manifest()).map((reference) => [reference.namespace, reference.schemaVersion]),
  )
  const policies: PolicyEvaluation['policies'][number][] = []
  ...
  for (const policy of [...options.policies].sort((left, right) => left.manifest.id.localeCompare(right.manifest.id))) {
    ...
    let rules: readonly PolicyRuleResult[]
...
function unavailableEvidence(
  ...
  capabilities: ReadonlyMap<string, Completeness>,
  schemas: ReadonlyMap<string, number>,
): string[] {
  ...
  for (const input of policy.manifest.inputs) {
    ...
    if (
      ...
      reasons.push(
        `Required schema ${input.namespace}@${input.minimumVersion}-${input.maximumVersion} is ${version ?? 'missing'}.`,
      )
    }
  }
  return reasons.sort()
...
function validateManifest(manifest: AnalysisPolicy['manifest'], seen: Set<string>): void {
  if (seen.has(manifest.id)) throw new Error(`Policy ${manifest.id} is installed more than once.`)
  ...
  const overlap =
    manifest.scopedCapabilities?.filter((capability) =>
      manifest.requiresCapabilities.includes(capability),
    ) ?? []
  if (
    overlap.length ||
...
function normalizeRules(
  ...
  actual: readonly PolicyRuleResult[],
  scopedEvidence: boolean,
): readonly PolicyRuleResult[] {
...
function aggregate(values: readonly PolicyRuleStatus[]): PolicyRuleStatus {
