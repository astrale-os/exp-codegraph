...
export function createBoundedFileCacheStore(options: BoundedFileCacheOptions): BinaryCacheStore {
  ...
  return {
    ...
    async save(value) {
      if (value.byteLength > options.maxEntryBytes) return
      const temporary = join(
        directory,
        `.${options.key}.${process.pid}.${randomUUID()}${TEMPORARY_SUFFIX}`,
      )
      try {
...
async function replaceFile(source: string, target: string): Promise<void> {
  try {
    ...
    if (!replaceRequiresRemoval(error)) throw error
...
function replaceRequiresRemoval(error: unknown): boolean {
  if (typeof error !== 'object' || error === null || !('code' in error)) return false
  return error.code === 'EEXIST' || error.code === 'EPERM'
}

export function defaultTypeSpecCacheDirectory(
  environment: NodeJS.ProcessEnv = process.env,
  platform: NodeJS.Platform = process.platform,
  home: string = homedir(),
): string {
  if (environment.ASTRALE_TYPESPEC_CACHE_DIR) return resolve(environment.ASTRALE_TYPESPEC_CACHE_DIR)
  if (environment.XDG_CACHE_HOME)
    return resolve(environment.XDG_CACHE_HOME, 'astrale-typespec', 'v2')
  if (platform === 'darwin') return join(home, 'Library', 'Caches', 'astrale-typespec', 'v2')
  if (platform === 'win32')
    return join(
      environment.LOCALAPPDATA ? resolve(environment.LOCALAPPDATA) : join(home, 'AppData', 'Local'),
      'astrale-typespec',
      'v2',
    )
...
async function prune(directory: string, options: BoundedFileCacheOptions): Promise<void> {
  let entries
  try {
  ...
  await Promise.all(
    entries
      ...
      .map(async (entry) => {
        ...
        try {
          if (now - (await stat(file)).mtimeMs >= STALE_TEMPORARY_AGE_MS) await removeFile(file)
        } catch {
