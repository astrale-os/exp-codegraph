// Native compiler RSS is not represented by the parent Node process. Keep projects serial so a
// larger corpus cannot multiply resident compiler memory and trigger an operating-system kill.
...
export function groupApplicationCompilerProjects(
...
export async function mapApplicationCompilerProjects<Input, Output>(
  ...
  await Promise.all(
    Array.from(
      { length: Math.min(MAXIMUM_CONCURRENT_COMPILER_PROJECTS, inputs.length) },
      async () => {
        while (true) {
          const index = next++
...
export class ApplicationCompilerRoutingIndex {
  readonly #routes = new Map<
    string,
    Map<string, TypeScriptModuleRouting['modules'][number]>
  >()
  ...
  update(project: string, routing: TypeScriptModuleRouting | undefined): void {
    ...
    const routes = routing.complete
      ? new Map<string, TypeScriptModuleRouting['modules'][number]>()
      : new Map(this.#routes.get(project) ?? [])
    for (const route of routing.modules) routes.set(route.module, route)
    this.#routes.set(project, routes)
  }

  affected(
    projects: readonly CompilerProject[],
    changes: readonly NativeSourceChange[] | undefined,
    conservative: boolean,
  ): readonly CompilerProject[] {
    if (conservative || !changes?.length || changes.some((change) => change.kind !== 'change')) {
      return projects
    }
    if (projects.some(([project]) => !this.#routes.has(project))) return projects
    const projectByModule = new Map(
      projects.flatMap(([project, modules]) => modules.map((module) => [module.id, project] as const)),
    )
    ...
    for (const change of changes) {
      const owners = projects.flatMap(([project, modules]) => {
        ...
        const bounded = modules.some(
          (module) => change.path === module.root || change.path.startsWith(`${module.root}/`),
        )
    ...
    while (expanded) {
      ...
      for (const [project, routes] of this.#routes) {
        if (affected.has(project)) continue
        if (
          [...routes.values()].some((route) =>
            route.dependencies.some((module) => affected.has(projectByModule.get(module) ?? '')),
          )
        ) {
  ...
  retained(
    projects: readonly CompilerProject[],
    residentModules: readonly string[] | undefined,
  ): ReadonlySet<string> {
