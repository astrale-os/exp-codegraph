...
export function projectedApplicationCheckpointSources(
  owners: readonly ApplicationCheckpointProjectionOwner[],
  projection: NonNullable<ApplicationCheckpointExpectation['projection']>,
): ReadonlySet<string> {
  const dependencies = new Map(
    owners.map(({ source, dependencies }) => [source, new Set(dependencies)] as const),
  )
  const selected = new Set(projection.requested.flatMap((target) =>
    applicationSelectionOwners(owners, target).map(({ source }) => source),
  ))
  if (projection.includeDependents) {
    ...
    while (changed) {
      ...
      for (const [source, imports] of dependencies) {
        if (selected.has(source) || ![...imports].some((item) => selected.has(item))) continue
        selected.add(source)
        changed = true
      }
    }
...
export function applicationCheckpointSpecificationDependencies(
