...
import { VERIFICATION_ENDPOINT, VERIFICATION_HEADER, VERIFICATION_PROTOCOL } from '../qualification.ts'
...
export async function handleVerificationHttp(
  ...
  if (url.pathname !== VERIFICATION_ENDPOINT) return false
  ...
  let input: VerificationRunRequest
  ...
  try {
    const result = await execute(input, snapshots[0]! as `application:${string}`)
...
function parseRequest(value: unknown): VerificationRunRequest {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('Verification request must be an object.')
  }
  const input = value as Record<string, unknown>
  const unknown = Object.keys(input).find(
    (key) => key !== 'protocol' && key !== 'source' && key !== 'revision',
  )
  if (unknown) throw new Error(`Verification request contains unsupported property ${unknown}.`)
...
async function requestJson(request: IncomingMessage): Promise<unknown> {
  ...
  for await (const value of request) {
    ...
    size += chunk.length
    if (size > MAX_REQUEST_BYTES) throw new Error('Verification request is too large.')
    chunks.push(chunk)
  }
  try {
...
function rejectionStatus(code: VerificationRejectionCode): number {
  if (code === 'SOURCE_CHANGED') return 409
  if (code === 'SOURCE_NOT_FOUND' || code === 'VERIFIER_MISSING') return 404
  if (code === 'SPEC_INVALID') return 422
  return 500
}
...
function reject(
  response: ServerResponse,
  status: number,
  code: 'REQUEST_INVALID' | 'EXECUTION_FAILED',
  message: string,
): void {
  send(response, status, { protocol: VERIFICATION_PROTOCOL, status: 'rejected', code, message })
}

function send(response: ServerResponse, status: number, value: VerificationRunResponse): void {
