...
import { SOURCE_EDIT_ENDPOINT, SOURCE_EDIT_HEADER } from '../editing.ts'

export async function handleSourceEditHttp(
  request: IncomingMessage,
  response: ServerResponse,
  execute: (request: SourceEditRequest, snapshot: `application:${string}`) => Promise<SourceEditResponse>,
): Promise<boolean> {
  if (!request.url) return false
  const url = new URL(request.url, 'http://127.0.0.1')
  if (url.pathname !== SOURCE_EDIT_ENDPOINT) return false
...
function statusOf(result: SourceEditResponse): number {
  if (result.status === 'saved') return 200
  if (result.status === 'conflict') return 409
  if (result.message === 'Application snapshot changed; reload the catalog.') return 409
  return result.message === 'Specification source not found.' ? 404 : 400
}

function applicationSnapshot(url: URL): `application:${string}` | undefined {
  const values = url.searchParams.getAll('snapshot')
  const value = values.length === 1 ? values[0] : undefined
  return value && /^application:[a-f\d]{64}$/u.test(value)
    ? value as `application:${string}`
    : undefined
}
...
function parseRevision(value: string | string[] | undefined): string | undefined {
...
async function requestText(request: IncomingMessage): Promise<string> {
  ...
  for await (const value of request) {
    ...
    size += chunk.length
    if (size > MAX_FILE_BYTES) throw new Error(`File exceeds ${MAX_FILE_BYTES} bytes.`)
    chunks.push(chunk)
  }
  try {
...
function send(response: ServerResponse, status: number, value: SourceEditResponse): void {
