...
export async function compileApplicationModuleBindingsIsolated(options: {
  ...
  const compilations = [await runWorker(
    options.root,
    options.requests,
    options.ownershipRequests,
    options.signal,
  )]
  return {
    ...
    workerResidentUpperBoundBytes: Math.max(
      0,
      ...compilations.map((value) => value.workerResidentUpperBoundBytes),
    ),
  }
}
...
function runWorker(
  ...
  ownershipRequests?: readonly ApplicationModuleBindingRequest[],
  signal?: AbortSignal,
): Promise<ApplicationModuleBindingCompilation> {
  return new Promise((resolvePromise, reject) => {
    ...
    let stderrBytes = 0
    let settled = false
    const finish = (error?: unknown, result?: ApplicationModuleBindingCompilation) => {
      if (settled) return
      settled = true
      clearTimeout(timer)
    ...
    const timer = setTimeout(
      () => fail(`Application module binding worker exceeded ${TIMEOUT_MS} ms.`),
      TIMEOUT_MS,
    )
    child.stdout.on('data', (chunk: Buffer) => {
      stdoutBytes += chunk.byteLength
      if (stdoutBytes > MAXIMUM_OUTPUT_BYTES) return fail('Application module binding worker output exceeded its limit.')
      stdout.push(chunk)
    })
    child.stderr.on('data', (chunk: Buffer) => {
      stderrBytes += chunk.byteLength
      if (stderrBytes > MAXIMUM_STDERR_BYTES) return fail('Application module binding worker diagnostics exceeded their limit.')
      stderr.push(chunk)
    })
    ...
    child.on('close', (code, closeSignal) => {
      ...
      if (code !== 0) {
        return finish(new Error(
          `Application module binding worker exited with ${closeSignal ?? code ?? 'unknown status'}: ${Buffer.concat(stderr).toString('utf8').trim()}`,
        ))
      }
      try {
        const result = JSON.parse(Buffer.concat(stdout).toString('utf8')) as ApplicationModuleBindingCompilation
...
function sum(
  values: readonly ApplicationModuleBindingCompilation[],
  key: 'programMs' | 'diagnosticsMs' | 'surfaceMs' | 'exportsMs' | 'dependenciesMs' | 'evidenceMs',
): number {
...
if (process.argv.includes(WORKER_ARGUMENT)) {
  ...
  const options = JSON.parse(Buffer.concat(chunks).toString('utf8')) as {
    readonly root: string
    readonly requests: readonly ApplicationModuleBindingRequest[]
    readonly ownershipRequests?: readonly ApplicationModuleBindingRequest[]
  }
