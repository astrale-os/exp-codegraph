...
import { DEFAULT_WORKSPACE_CHECKPOINT_LIMITS } from './model.ts'

export const SHA256 = /^[a-f0-9]{64}$/u
export const MAX_SCOPE_LENGTH = 128
export const MAX_ARTIFACT_KEY_BYTES = 16 * 1024

export function normalizeLimits(options: FileWorkspaceCheckpointStoreOptions): NormalizedLimits {
  const limits = {
...
export function preparePublication(
  ...
  if (!isRecord(input) || !isRecord(input.manifest)) throw new TypeError('Checkpoint manifest is required.')
  ...
  for (const item of rawArtifacts) {
    ...
    if (seenKeys.has(item.key)) throw new TypeError(`Checkpoint artifact key is duplicated: ${item.key}`)
    seenKeys.add(item.key)
    if (!(item.bytes instanceof Uint8Array)) throw new TypeError(`Checkpoint artifact ${item.key} is not bytes.`)
...
function buildManifest(
  source: WorkspaceCheckpointManifestInput,
  scope: string,
  ...
  return manifest as WorkspaceCheckpointManifest
}

function normalizeArtifacts(input: WorkspaceCheckpointArtifacts): WorkspaceCheckpointArtifactInput[] {
  ...
  if (Array.isArray(input)) {
    return input.map((entry) => {
      if (Array.isArray(entry)) {
        if (entry.length !== 2) throw new TypeError('Checkpoint artifact tuples must have two items.')
  ...
  if (!isRecord(input)) throw new TypeError('Checkpoint artifacts must be a map, object, or list.')
  const objectInput = input as Readonly<Record<string, Uint8Array>>
...
export function validateStoredManifest(
  value: unknown,
  scope: string,
  limits: NormalizedLimits,
): WorkspaceCheckpointManifest {
  if (!isRecord(value)) throw new TypeError('Checkpoint manifest is not an object.')
  validateManifestFields(value, scope, true)
  if (!Array.isArray(value.artifacts)) throw new TypeError('Checkpoint manifest artifacts are invalid.')
  if (value.artifacts.length > limits.maxArtifacts) throw new RangeError('Checkpoint artifact count is too large.')
  ...
  for (const item of value.artifacts) {
    if (!isRecord(item)) throw new TypeError('Checkpoint artifact descriptor is invalid.')
    ...
    validateArtifactKey(item.key)
    if (!SHA256.test(item.digest)) throw new TypeError('Checkpoint artifact digest is invalid.')
    if (!Number.isSafeInteger(item.bytes) || item.bytes < 0) {
      throw new TypeError('Checkpoint artifact byte count is invalid.')
    }
    if (item.bytes > limits.maxArtifactBytes) throw new RangeError('Checkpoint artifact is too large.')
    if (seenKeys.has(item.key)) throw new TypeError('Checkpoint artifact keys are duplicated.')
  ...
  } as unknown as WorkspaceCheckpointManifest
}

function validateManifestFields(value: Record<string, unknown>, scope: string, stored: boolean): void {
  ...
  if (value.scope !== scope) throw new TypeError('Checkpoint manifest scope is invalid.')
  if (!Object.hasOwn(value, 'payload')) throw new TypeError('Checkpoint manifest payload is missing.')
  ...
  if (!hasFingerprint && !hasProducer) throw new TypeError('Checkpoint producer fingerprint is missing.')
  ...
  if (value.producer !== undefined) assertJsonValue(value.producer)
  if (stored && !Array.isArray(value.artifacts)) throw new TypeError('Checkpoint manifest artifacts are missing.')
}

function assertJsonValue(value: unknown, active = new WeakSet<object>()): asserts value is JsonValue {
  if (value === null || typeof value === 'string' || typeof value === 'boolean') return
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new TypeError('Checkpoint JSON values must be finite.')
    return
  }
  if (Array.isArray(value)) {
    if (active.has(value)) throw new TypeError('Checkpoint JSON values cannot be cyclic.')
    active.add(value)
    for (const item of value) assertJsonValue(item, active)
    active.delete(value)
    return
  }
  if (typeof value === 'object') {
    ...
    for (const [key, item] of Object.entries(value)) {
      if (item === undefined) throw new TypeError(`Checkpoint JSON value ${key} is undefined.`)
...
export function canonicalJson(value: unknown, active = new WeakSet<object>()): string {
  assertJsonValue(value)
  return canonicalJsonValue(value, active)
}

function canonicalJsonValue(value: JsonValue, active: WeakSet<object>): string {
  ...
  active.add(value)
  try {
    if (Array.isArray(value)) return `[${value.map((item) => canonicalJsonValue(item, active)).join(',')}]`
    return `{${Object.keys(value)
      .sort(compareCodePoints)
      .map((key) => `${canonicalString(key)}:${canonicalJsonValue((value as Record<string, JsonValue>)[key]!, active)}`)
      .join(',')}}`
  } finally {
    active.delete(value)
  }
...
export function throwIfAborted(signal: AbortSignal | undefined, error?: unknown): void {
  if (error !== undefined && isAbort(error)) throw error
  if (signal?.aborted) {
